...
BytesPerChar    EQU     2
PRINT_D_Graphics_Screen_HIG117:
...
!       LDA     _Var_PF04
        ...
        BSR     AtoGraphics_Screen_HIG117
        LDA     #$20        ; Blank
        BRA     AtoGraphics_Screen_HIG117 ; Print A on the screen and return, Done
@Print1000sGScreen:
        LDA     _Var_PF01
        ADDA    #$30
        BSR     AtoGraphics_Screen_HIG117
...
@Print1sGScreen:
        ...
        BSR     AtoGraphics_Screen_HIG117
        LDA     #$20        ; Blank
        BRA     AtoGraphics_Screen_HIG117 ; Print A on the screen and return

AtoGraphics_Screen_HIG117:
...
!       LDU   BEGGRP    ; U = screen start
        LEAU  Screen_Size_HIG117-(FontHeight-1)*BytesPerRow_HIG117,U      ; U = screen End - 7 rows
        PSHS  U         ; Save the value on the stack
        CMPX  ,S++      ; Compare X with the value on the stack, fix the stack
        BLO   >
        LDX   BEGGRP    ; X = Screen Start
        LEAX  Screen_Size_HIG117-BytesPerRow_HIG117*FontHeight,X  ; X = Screen Start + screen size - 1 row = bottom of the screen
        STX   GraphicCURPOS
!       CMPA  #$0D      ; Enter Key?
...
!       BSR   @GraphicTextDrawAatX  ; Go Draw character A on screen at X
        ...
        TFR   X,B       ; B = X (LSB)
        BITB  #BytesPerRow_HIG117-1      ; Is it the start of a new row?
        BNE   <         ; if not keep printing a blank
        LEAX  (FontHeight-1)*BytesPerRow_HIG117,X    ; Point at the start of the next row
        LEAU  (FontHeight-1)*BytesPerRow_HIG117,U    ; U = screen End
        PSHS  U         ; Save the value on the stack
        CMPX  ,S++      ; Compare X with the value on the stack, fix the stack
...
!       CMPB    #96             ; Is it a printable character?
        BLO     >               ; Yes, skip ahead
        CLRB                    ; Otherwise, make it a space
!       LDX     #CharJumpTable_HIG117 ; Get the address of the character jump table
