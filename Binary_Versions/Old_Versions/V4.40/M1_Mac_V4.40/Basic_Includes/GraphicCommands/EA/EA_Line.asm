...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_EA       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
!       JSR     SET_EA      ; Go draw the pixel on screen
...
!       JSR     SET_EA           ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_EA       ; If <=, then keep looping
