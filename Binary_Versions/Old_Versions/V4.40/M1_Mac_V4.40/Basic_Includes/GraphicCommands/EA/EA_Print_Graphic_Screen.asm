...
!       LDA     _Var_PF04
        ...
        BSR     AtoGraphics_Screen_EA
        LDA     #$20        ; Blank
        BRA     AtoGraphics_Screen_EA ; Print A on the screen and return, Done
@Print1000sGScreen:
        LDA     _Var_PF01
        ADDA    #$30
        BSR     AtoGraphics_Screen_EA
...
@Print1sGScreen:
        ...
        BSR     AtoGraphics_Screen_EA
        LDA     #$20        ; Blank
        BRA     AtoGraphics_Screen_EA ; Print A on the screen and return

AtoGraphics_Screen_EA:
...
!       LDU   BEGGRP    ; U = screen start
        LEAU  Screen_Size_EA-(FontHeight-1)*BytesPerRow_EA,U      ; U = screen End - 7 rows
        PSHS  U         ; Save the value on the stack
        CMPX  ,S++      ; Compare X with the value on the stack, fix the stack
        BLO   >
        LDX   BEGGRP    ; X = Screen Start
        LEAX  Screen_Size_EA-BytesPerRow_EA*FontHeight,X  ; X = Screen Start + screen size - 1 row = bottom of the screen
        STX   GraphicCURPOS
!       CMPA  #$0D      ; Enter Key?
...
!       CMPD  #BytesPerRow_EA          ; Is D less than BytesPerRow_EA?
        BLO   >             ; If yes, we're done
        SUBD  #BytesPerRow_EA          ; Subtract one full row
        BRA   <             ; Repeat until D < BytesPerRow_EA
; B now contains the column number
!       SUBB   #BytesPerRow_EA      ; B = B - ByttesPerRow_EA
        LDA   #$80      ; Blank
!       BSR   @GraphicTextDrawAatX  ; Go Draw character A on screen at X
        LEAX  BytesPerChar,X       ; Move to the next position
        INCB
        BNE   <         ; if not keep printing a blank
        LEAX  (FontHeight-1)*BytesPerRow_EA,X    ; Point at the start of the next row
        LEAU  (FontHeight-1)*BytesPerRow_EA,U    ; U = screen End
        PSHS  U         ; Save the value on the stack
        CMPX  ,S++      ; Compare X with the value on the stack, fix the stack
