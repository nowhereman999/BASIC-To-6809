...
!       LDA     _Var_PF04
        ...
        BSR     AtoGraphics_Screen_SG24
        LDA     #$20        ; Blank
        BRA     AtoGraphics_Screen_SG24 ; Print A on the screen and return, Done
@Print1000sGScreen:
        LDA     _Var_PF01
        ADDA    #$30
        BSR     AtoGraphics_Screen_SG24
...
@Print1sGScreen:
        ...
        BSR     AtoGraphics_Screen_SG24
        LDA     #$20        ; Blank
        BRA     AtoGraphics_Screen_SG24 ; Print A on the screen and return

AtoGraphics_Screen_SG24:
...
!       LDU   BEGGRP    ; U = screen start
        LEAU  Screen_Size_SG24-(FontHeight-1)*BytesPerRow_SG24,U      ; U = screen End - 7 rows
        PSHS  U         ; Save the value on the stack
        CMPX  ,S++      ; Compare X with the value on the stack, fix the stack
        BLO   >
        LDX   BEGGRP    ; X = Screen Start
        LEAX  Screen_Size_SG24-BytesPerRow_SG24*FontHeight,X  ; X = Screen Start + screen size - 1 row = bottom of the screen
        STX   GraphicCURPOS
!       CMPA  #$0D      ; Enter Key?
...
!       CMPD  #BytesPerRow_SG24          ; Is D less than BytesPerRow_SG24?
        BLO   >             ; If yes, we're done
        SUBD  #BytesPerRow_SG24          ; Subtract one full row
        BRA   <             ; Repeat until D < BytesPerRow_SG24
; B now contains the column number
!       SUBB   #BytesPerRow_SG24      ; B = B - ByttesPerRow_SG24
        LDA   #$80      ; Blank
!       BSR   @GraphicTextDrawAatX  ; Go Draw character A on screen at X
        LEAX  BytesPerChar,X       ; Move to the next position
        INCB
        BNE   <         ; if not keep printing a blank
        LEAX  (FontHeight-1)*BytesPerRow_SG24,X    ; Point at the start of the next row
        LEAU  (FontHeight-1)*BytesPerRow_SG24,U    ; U = screen End
        PSHS  U         ; Save the value on the stack
        CMPX  ,S++      ; Compare X with the value on the stack, fix the stack
...
@GraphicTextDrawAatX:
        STA     ,X              ; Draw the character  4
        STA     BytesPerRow_SG24,X ; Draw the character 5
        STA     BytesPerRow_SG24*2,X ; Draw the character 5
        STA     BytesPerRow_SG24*3,X ; Draw the character 5
        LEAX    BytesPerRow_SG24*8,X
        STA     -BytesPerRow_SG24*4,X ; Draw the character 8
        STA     -BytesPerRow_SG24*3,X ; Draw the character
        STA     -BytesPerRow_SG24*2,X ; Draw the character
        STA     -BytesPerRow_SG24*1,X ; Draw the character
