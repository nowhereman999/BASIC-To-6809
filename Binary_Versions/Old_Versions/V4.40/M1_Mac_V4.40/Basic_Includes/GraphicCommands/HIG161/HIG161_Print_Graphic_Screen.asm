...
PRINT_D_Graphics_Screen_HIG161:
...
!       LDA     _Var_PF04
        ...
        BSR     AtoGraphics_Screen_HIG161
        LDA     #$20        ; Blank
        BRA     AtoGraphics_Screen_HIG161 ; Print A on the screen and return, Done
@Print1000sGScreen:
        LDA     _Var_PF01
        ADDA    #$30
        BSR     AtoGraphics_Screen_HIG161
...
@Print1sGScreen:
        ...
        BSR     AtoGraphics_Screen_HIG161
        LDA     #$20        ; Blank
        BRA     AtoGraphics_Screen_HIG161 ; Print A on the screen and return

AtoGraphics_Screen_HIG161:
...
!       LDU   BEGGRP    ; U = screen start
        LEAU  Screen_Size_HIG161-(FontHeight-1)*BytesPerRow_HIG161,U      ; U = screen End - 7 rows
        PSHS  U         ; Save the value on the stack
        CMPX  ,S++      ; Compare X with the value on the stack, fix the stack
        BLO   >
        LDX   BEGGRP    ; X = Screen Start
        LEAX  Screen_Size_HIG161-BytesPerRow_HIG161*FontHeight,X  ; X = Screen Start + screen size - 1 row = bottom of the screen
        STX   GraphicCURPOS
!       BSR   @GraphicTextDrawAatX  ; Go Draw character A on screen at X
        LEAX  BytesPerChar,X       ; Move to the next position
;        TFR   X,D       ; D = X
;        BITB  #BytesPerRow_HIG161-1  ; Is it the start of a new row?
;        BNE   @GraphicsTextUpdateCursor ; No, update the cursor and exit
;        LEAX  (FontHeight-1)*BytesPerRow_HIG161,X    ; Point at the start of the next row
...
!       CMPB    #96             ; Is it a printable character?
        BLO     >               ; Yes, skip ahead
        CLRB                    ; Otherwise, make it a space
!       LDX     #CharJumpTable_HIG161 ; Get the address of the character jump table
