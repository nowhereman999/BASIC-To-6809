...
; decision= decision+ 4 * (x - y) + 10
        ...
        ADDD    decision
        STD     decision
        BRA     CircleLoop_HIG115          ; Loop until x <= y
; else:
...
; Plot (x + x_Center, y + y_Center)
        ...
        LDA     y0+1
        ADDA    y_Center+1
        BSR     DoSetYXCheck_HIG115            ; Plot the pixel
; Plot (-x + x_Center, y + y_Center)
        LDB     x0+1
        LSRB                            ; Scale B
        NEGB
...
DoSetYXCheck_HIG115:
!           CMPA    #ScreenHeight_HIG115-1
            BLS     @GoodA
; If we get here the value is not in screen's range
			CMPA	#255-((255-ScreenHeight_HIG115-1)/2) 
			BLS		>			; If we are <= the midpoint beyond the screen size then use max value
			CLRA				; otherwise make it zero
			BRA		@GoodA
!           LDA     #ScreenHeight_HIG115-1
@GoodA      CMPB    #ScreenWidth_HIG115-1
            BLS     @GoodB
; If we get here the value is not in screen's range
			CMPB	#255-((255-ScreenWidth_HIG115-1)/2) 
			BLS		>			; If we are <= the midpoint beyond the screen size then use max value
			CLRB				; otherwise make it zero
			BRA		@GoodB
!           LDB     #ScreenWidth_HIG115-1
@GoodB      JMP     SET_HIG115    ; go set the pixel
