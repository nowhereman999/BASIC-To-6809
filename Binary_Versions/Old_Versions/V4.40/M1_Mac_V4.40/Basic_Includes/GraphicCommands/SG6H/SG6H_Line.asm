...
LINE_SG6H:
        ...
        STA     C_SG6H           ; Save the colour for the line command to use
        CLRA
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop4_SG6H       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
BoxDrawDot0_SG6H:
!       JSR     DoSetSaveAB_SG6H ; Go draw the pixel on screen
...
!       JSR     DoSetSaveAB_SG6H     ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_SG6H   ; If <=, then keep looping
