; Graphics commands for screen mode HIG112
; Resolution of 128x192x2 colours
ScreenWidth_HIG112    EQU 128
ScreenHeight_HIG112   EQU 192
BytesPerRow_HIG112    EQU ScreenWidth_HIG112/8
Screen_Size_HIG112    EQU $0C00
;
; Do SET
; Enter with:
...
DoSet_HIG112:
        ...
        BEQ     RESET_HIG112      ; If it's not zero then draw white dot
SET_HIG112:
...
!       LSRA
        ...
        LDU     #PixelTable_HIG112 ; U points to the table of pixel values
...
!       LSRA
        ...
        LDB     1,S             ; Get B back which has the pixel number
        ANDB    #%00000111      ; B = Pixel value (0 to 7)
        LDU     #PixelTableClear_HIG112 ; U points to the table of pixel values
        LDA     ,X              ; Get existing pixels in this byte from the screen
        ANDA    B,U             ; AND original Byte value with the pixel the user wants to cleared
        STA     ,X              ; Write new byte to the screen
        PULS    D,PC            ; Restore D,X & U and return

PixelTableClear_HIG112:
...
GCLS_HIG112:
        LDX     BEGGRP
        LEAU    Screen_Size_HIG112,X ; Screen Size
        STU     GCLSComp_HIG112+1 ; Self mod end address
        TSTB
        BEQ     >
        LDB     #$FF
!       TFR     B,A     ; A = B
