...
; Test for a horizontal line
        ...
        BNE     LineNotHorizontal_HIG164  ; If they aren't the same then go draw a line normally
; Get the number of bytes between pixels
        LDB     endX+1          ; B = ending x coordinate
        CMPB    startX+1        ; Compare with starting x coordinate
        BHI     LineHorizontalSET_HIG164    ; If A is positive then go draw a line normally
        BEQ     LineDrawDot1_HIG164    ; If A is zero then go PSET one single pixel
        LDA     startX+1          ; Otherwise flip the startx and endx coordinates
        LDB     endX+1          ; and store them in the variables
        STA     endX+1          ;
        STB     startX+1        ;

LineHorizontalSET_HIG164:
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_HIG164       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
!       STA     B,X
        ...
        CMPB    #127
        BNE     <
        LEAX    BytesPerRow_HIG164,X    ; Move down a row
        TST     1,S
