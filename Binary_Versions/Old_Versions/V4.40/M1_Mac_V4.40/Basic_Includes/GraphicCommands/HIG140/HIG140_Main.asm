; Graphics commands for screen mode HIG140
; Resolution of 320x200x2 colours
ScreenWidth_HIG140    EQU 320
ScreenHeight_HIG140   EQU 200
BytesPerRow_HIG140    EQU ScreenWidth_HIG140/8
Screen_Size_HIG140    EQU $1F40
;
; SET a dot on screen
; Enter with:
; A = y coordinate
...
DoSet_HIG140:
        ...
        BEQ     RESET_HIG140      ; If it's not zero then draw white dot
SET_HIG140:
        ...
        LDU     #PixelTable_HIG140 ; U points to the table of pixel values
...
RESET_HIG140:
        ...
        LDB     2,S             ; Get B back which has the pixel number
        ANDB    #%00000111      ; B = Pixel value (0 to 7)
        LDU     #PixelTableClear_HIG140 ; U points to the table of pixel values
        LDA     ,X              ; Get existing pixels in this byte from the screen
        ANDA    B,U             ; AND original Byte value with the pixel the user wants to cleared
        STA     ,X              ; Write new byte to the screen
        PULS    A,X,PC            ; Restore D,X & U and return

PixelTableClear_HIG140:
...
GCLS_HIG140:
        LDX     BEGGRP
        LEAU    Screen_Size_HIG140,X ; Screen Size
        STU     GCLSComp_HIG140+1 ; Self mod end address
        TSTB
        BEQ     >
        LDB     #$FF
!       TFR     B,A     ; A = B
