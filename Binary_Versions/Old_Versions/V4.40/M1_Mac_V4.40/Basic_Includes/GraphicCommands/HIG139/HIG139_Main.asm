; Graphics commands for screen mode HIG139
; Resolution of 320x192x2 colours
ScreenWidth_HIG139    EQU 320
ScreenHeight_HIG139   EQU 192
BytesPerRow_HIG139    EQU ScreenWidth_HIG139/8
Screen_Size_HIG139    EQU $1E00
;
; SET a dot on screen
; Enter with:
; A = y coordinate
...
DoSet_HIG139:
        ...
        BEQ     RESET_HIG139      ; If it's not zero then draw white dot
SET_HIG139:
        ...
        LDU     #PixelTable_HIG139 ; U points to the table of pixel values
...
RESET_HIG139:
        ...
        LDB     2,S             ; Get B back which has the pixel number
        ANDB    #%00000111      ; B = Pixel value (0 to 7)
        LDU     #PixelTableClear_HIG139 ; U points to the table of pixel values
        LDA     ,X              ; Get existing pixels in this byte from the screen
        ANDA    B,U             ; AND original Byte value with the pixel the user wants to cleared
        STA     ,X              ; Write new byte to the screen
        PULS    A,X,PC            ; Restore D,X & U and return

PixelTableClear_HIG139:
...
GCLS_HIG139:
        LDX     BEGGRP
        LEAU    Screen_Size_HIG139,X ; Screen Size
        STU     GCLSComp_HIG139+1 ; Self mod end address
        TSTB
        BEQ     >
        LDB     #$FF
!       TFR     B,A     ; A = B
