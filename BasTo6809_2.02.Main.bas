...
count = _CommandCount
If count = 0 Then
    Print "BasTo6809 - BASIC to 6809 Assembly converter V"; V$; " By Glen Hewlett - https://github.com/nowhereman999"
    Print
...
If InStr(CompilerVersion$, "[MACOSX]") > 0 Or InStr(CompilerVersion$, "[LINUX]") > 0 Then
    Shell "./BasTo6809_" + V$ + ".1.Tokenizer " + c$ + s$ + o$ + b$ + Verbose$ + p$ + OutName$
    ...
    Shell "./BasTo6809_" + V$ + ".2.Compile " + c$ + s$ + o$ + b$ + Verbose$ + KeepTempFiles$ + OutName$
Else
    Shell ".\BasTo6809_" + V$ + ".1.Tokenizer.exe " + c$ + s$ + o$ + b$ + Verbose$ + p$ + OutName$
    ' We now have the BASIC program in a good tokenized format as the file BasicTokenized.bin
    ' Call the compiler with a few command line options to pass through to the compiler
    Shell ".\BasTo6809_" + V$ + ".2.Compile.exe " + c$ + s$ + o$ + b$ + Verbose$ + KeepTempFiles$ + OutName$
End If
System
