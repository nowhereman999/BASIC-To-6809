...
Print
Print "Usage: PNGtoCCSprite -gx [-scroll] [-sx] [-axx] [-bx] [-pxxx] [-sb] [-cx] [-dx] [-x] [-blkxx] [-makepal] [-usepal] [-v#] InName.png"
Print "Where:"
...
Print "-scroll  - Will generate a sprite to be used with a scrolling background (only use this option with a background that will scroll horizontally)"
...
Print "-c3        3 = Convert the .PNG file into a .ASM assembly file with FCB statements"
...
Print "-sb      - Scrollable Background - Convert the .PNG file into a format that can be scrolled on the CoCo 3"
...
For check = 1 To count
    ...
    If LCase$(Left$(N$, 3)) = "-sb" Then MakeScrollableBackground = 1: GoTo 120
...
If SaveRestoreBackground = 2 Then
    If Pattern = -1 Then Print "ERROR: Must use -pxxx to with option -b2 to set the byte pattern to fill when restoring the sprite": GoTo 99
End If
...
If MakeScrollableBackground = 1 Then GoTo MakeScrollingBackground
...
GoSub NumAsString 'Convert number in N to a string without spaces as N$ - a value of zero is ignored comes back as N$=""
If ScrollMode = 0 Then
    Instr1$ = "LDB": Instr2$ = "#" + N$: Com$ = "Amount of bytes to move down each row": GoSub DoOutput ' Print the output to file #2
End If
...
'Make a scrollable background image which will be 512 pixels wide
MakeScrollingBackground:
Dim DitherOut(imageWidth - 1, imageHeight - 1) As _Unsigned _Byte
...
If Colours = 16 Then
    ...
    For y = 0 To imageHeight - 1
        XPos = 0
        For x = 0 To imageWidth * 4 - 1 Step 8
            PixelsValue = 0
            If SpriteOriginal$(x, y) = "1" Then PixelsValue = 128
            If SpriteOriginal$(x + 1, y) = "1" Then PixelsValue = PixelsValue + 64
            If SpriteOriginal$(x + 2, y) = "1" Then PixelsValue = PixelsValue + 32
            If SpriteOriginal$(x + 3, y) = "1" Then PixelsValue = PixelsValue + 16
            If SpriteOriginal$(x + 4, y) = "1" Then PixelsValue = PixelsValue + 8
            If SpriteOriginal$(x + 5, y) = "1" Then PixelsValue = PixelsValue + 4
            If SpriteOriginal$(x + 6, y) = "1" Then PixelsValue = PixelsValue + 2
            If SpriteOriginal$(x + 7, y) = "1" Then PixelsValue = PixelsValue + 1
            DitherOut(XPos, y) = PixelsValue
            XPos = XPos + 1
        Next x
    Next y
...
chunks = Int(((imageHeight * 256) + &H2000 - 1) / &H2000)
For i = 0 To chunks - 1
    StartLocation = &HFFA2
    Blocksize = 1
    DiskOut(p) = 0: p = p + 1 ' Preamble value 0 = data, 255 = Last block
    ...
    StartLocation = &H4000
    Blocksize = &H2000
    If (i + 1) * &H2000 > fileSize Then
        Blocksize = fileSize - i * &H2000
    End If
    ...
    While c <> Blocksize
        For x = 0 To 255
            DiskOut(p) = DitherOut(x, y): p = p + 1: c = c + 1
        Next x
        y = y + 1
    Wend
Next i
