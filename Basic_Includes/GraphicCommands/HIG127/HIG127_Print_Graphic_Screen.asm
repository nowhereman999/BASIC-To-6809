...
BytesPerChar    EQU     4
PRINT_D_Graphics_Screen_HIG127:
...
!       LDA     _Var_PF04
        ...
        BSR     AtoGraphics_Screen_HIG127
        LDA     #$20        ; Blank
        BRA     AtoGraphics_Screen_HIG127 ; Print A on the screen and return, Done
@Print1000sGScreen:
        LDA     _Var_PF01
        ADDA    #$30
        BSR     AtoGraphics_Screen_HIG127
...
@Print1sGScreen:
        ...
        BSR     AtoGraphics_Screen_HIG127
        LDA     #$20        ; Blank
        BRA     AtoGraphics_Screen_HIG127 ; Print A on the screen and return

AtoGraphics_Screen_HIG127:
...
!       LDU   BEGGRP    ; U = screen start
        LEAU  Screen_Size_HIG127-(FontHeight-1)*BytesPerRow_HIG127,U      ; U = screen End - 7 rows
        PSHS  U         ; Save the value on the stack
        CMPX  ,S++      ; Compare X with the value on the stack, fix the stack
        BLO   >
        LDX   BEGGRP    ; X = Screen Start
        LEAX  Screen_Size_HIG127-BytesPerRow_HIG127*FontHeight,X  ; X = Screen Start + screen size - 1 row = bottom of the screen
        STX   GraphicCURPOS
!       CMPA  #$0D      ; Enter Key?
...
!       CMPD  #BytesPerRow_HIG127          ; Is D less than BytesPerRow_HIG127?
        BLO   >             ; If yes, we're done
        SUBD  #BytesPerRow_HIG127          ; Subtract one full row
        BRA   <             ; Repeat until D < BytesPerRow_HIG127
; B now contains the column number
!       SUBB   #BytesPerRow_HIG127      ; B = B - ByttesPerRow_HIG127
        LDA   #$20      ; Blank
!       BSR   @GraphicTextDrawAatX  ; Go Draw character A on screen at X
        LEAX  BytesPerChar,X       ; Move to the next position
        INCB
        BNE   <         ; if not keep printing a blank
        LEAX  (FontHeight-1)*BytesPerRow_HIG127,X    ; Point at the start of the next row
        LEAU  (FontHeight-1)*BytesPerRow_HIG127,U    ; U = screen End
        PSHS  U         ; Save the value on the stack
        CMPX  ,S++      ; Compare X with the value on the stack, fix the stack
...
!       CMPB    #96             ; Is it a printable character?
        BLO     >               ; Yes, skip ahead
        CLRB                    ; Otherwise, make it a space
!       LDX     #CharJumpTable_HIG127 ; Get the address of the character jump table
