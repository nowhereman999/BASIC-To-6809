...
LINE_HIG149:
        ...
        BNE     DoLINE_HIG149     ; If it's not zero then draw white line
RESETLINE_HIG149:
        LDU     #RESET_HIG149     ; Self mod address of RESET routine
        BRA     >
DoLINE_HIG149:
        LDU     #SET_HIG149       ; Self mod address of SET routine
!       STU     LineDrawDot2_HIG149+1  ; Address where to draw the dot
...
; Draw the bits at the beginning that aren't a complete byte
        ...
        LDB     #BytesPerRow_HIG149 ; MUL A by the number of bytes per row
        MUL      
...
* Fill the pixels on the left side of the line then draw bytes
        LDA     LineColour      ; Get the line colour
        BNE     LineHorizontalSET_HIG149     ; If it's not zero then draw white line
...
* Do the RESET version of a Horizontal line
        ...
        LDU     #LineDrawSetTableStart_HIG149
...
* Draw the pixels on the right side of the line
        ...
        LDU     #LineDrawSetTableEnd_HIG149
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_HIG149       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
BOX_HIG149:
        LDA     LineColour      ; Get the line colour
        BNE     DoBOX_HIG149      ; If it's not zero then draw white Box
PRESETBOX_HIG149:
        LDD     #RESET_HIG149        ; Self mod address of PRESET routine
        BRA     >
DoBOX_HIG149:
        LDD     #SET_HIG149          ; Self mod address of PSET routine
!       STD     BoxDrawDot0_HIG149+1
        STD     BoxDrawDot1_HIG149+1
        STD     BoxDrawDot2_HIG149+1
        STD     BoxDrawDot3_HIG149+1
...
BoxDrawDot0_HIG149:
!       JSR     SET_HIG149      ; Go draw the pixel on screen
...
BoxFill_HIG149:
        LDA     LineColour      ; Get the line colour
        BNE     DoBoxFill_HIG149  ; If it's not zero then fill a white Box
PRESETBoxFill_HIG149:
        LDD     #RESET_HIG149          ; Self mod address of PSET routine
        BRA     >
DoBoxFill_HIG149:
        LDD     #SET_HIG149          ; Self mod address of PSET routine
!       STD     BoxFillDrawDot0_HIG149+1
...
!       JSR     SET_HIG149      ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_HIG149 ; If <=, then keep looping
