; Graphics commands for screen mode DMAGraphic
; Resolution of 256x192x2 colours
ScreenWidth_DMAGraphic    EQU 256
ScreenHeight_DMAGraphic   EQU 192
BytesPerRow_DMAGraphic    EQU ScreenWidth_DMAGraphic/8
Screen_Size_DMAGraphic    EQU $1800
;
; SET a dot on screen
; A=y value, B=x value
DoSet_DMAGraphic:
        TST     LineColour      ; Get the Set colour
        BEQ     RESET_DMAGraphic      ; If it's not zero then draw white dot
SET_DMAGraphic:
        ...
        LDU     #PixelTable_DMAGraphic ; U points to the table of pixel values
...
RESET_DMAGraphic:
        ...
        LDB     1,S             ; Get B back which has the pixel number
        ANDB    #%00000111      ; B = Pixel value (0 to 7)
        LDU     #PixelTableClear_DMAGraphic ; U points to the table of pixel values
        LDA     ,X              ; Get existing pixels in this byte from the screen
        ANDA    B,U             ; AND original Byte value with the pixel the user wants to cleared
        STA     ,X              ; Write new byte to the screen
        PULS    D,PC            ; Restore D,X & U and return

PixelTableClear_DMAGraphic:
...
GCLS_DMAGraphic:
        LDX     BEGGRP
        LEAU    Screen_Size_DMAGraphic,X ; Screen Size
        STU     GCLSComp_DMAGraphic+1 ; Self mod end address
        TSTB
        BEQ     >
        LDB     #$FF
!       TFR     B,A     ; A = B
