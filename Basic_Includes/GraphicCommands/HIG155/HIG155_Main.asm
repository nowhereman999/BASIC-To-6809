; Graphics commands for screen mode HIG155
; Resolution of 640x200x2 colours
ScreenWidth_HIG155    EQU 640
ScreenHeight_HIG155   EQU 200
BytesPerRow_HIG155    EQU ScreenWidth_HIG155/8
Screen_Size_HIG155    EQU $3E80
;
; SET a dot on screen
; Enter with:
; A = y coordinate
...
; y location is 0 to 191 (200 rows)
;
; A=y value, X=x value
RESET_HIG155:
        PSHS    A,X             ; Save x & y coordinate on the stack
        ...
        LDB     2,S             ; Get B back which has the pixel number
        ANDB    #%00000111      ; B = Pixel value (0 to 7)
        LDU     #PixelTableClear_HIG155 ; U points to the table of pixel values
        LDA     ,X              ; Get existing pixels in this byte from the screen
        ANDA    B,U             ; AND original Byte value with the pixel the user wants to cleared
        STA     ,X              ; Write new byte to the screen
        PULS    A,X,PC            ; Restore D,X & U and return

PixelTableClear_HIG155:
...
GCLS_HIG155:
        LDX     BEGGRP
        LEAU    Screen_Size_HIG155,X ; Screen Size
        STU     GCLSComp_HIG155+1 ; Self mod end address
        TSTB
        BEQ     >
        LDB     #$FF
!       TFR     B,A     ; A = B
