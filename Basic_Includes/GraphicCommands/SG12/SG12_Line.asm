...
LINE_SG12:
        ...
        STA     C_SG12          ; Save the colour for the line command to use
        CLRA
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop4_SG12       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
!       JSR     SET_SG12      ; Go draw the pixel on screen
...
!       JSR     SET_SG12           ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_SG12       ; If <=, then keep looping
