...
Calc5120x192:
        ...
        LDD     ViewPortX       ; Get the viewport (MSB) location
        INCA
        CMPD    2,S
        BLO     @AlsoNotviewable
; Sprite is in viewable range draw it to the same playfield as the viewport
        DECA
        CMPA    2,S
        BEQ     @Same1          ; If they are the same then all good to use as is
        BLO     @Viewlower      ; if the viewport is lower then make the sprite show up on the viewport by moving it back
        BRA     *
@Same1:
        ANDA    #1
        STA     MatchViewport
