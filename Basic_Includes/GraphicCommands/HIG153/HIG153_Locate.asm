...
!       CMPD    #BytesPerRow_HIG153-1 ; Check if it's beyond the right side of the screen
        BLE     >               ; Skip ahead if good
        LDD     #BytesPerRow_HIG153-1 ; Make it the right most column
!       STD     x0              ; Save the x value

        LDD     y0              ; Get the y value in D
        BPL     >               ; Skip ahead if positive
        LDD     #$0000          ; Otherwise, make it zero
!       CMPD    #ScreenHeight_HIG153-FontHeight          ; Check if it's beyond the bottom of the screen
        BLE     >               ; Skip ahead if good
        LDD     #ScreenHeight_HIG153-FontHeight          ; Make it the bottom row

!       LDA     #BytesPerRow_HIG153 ; A= bytes per row
