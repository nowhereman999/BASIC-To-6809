...
LINE_FG2R:
        ...
        BNE     DoLINE_FG2R     ; If it's not zero then draw white line
RESETLINE_FG2R:
        LDX     #RESET_FG2R     ; Self mod address of PSET routine
        BRA     >
DoLINE_FG2R:
        LDX     #SET_FG2R       ; Self mod address of PSET routine
!       STX     LineDrawDot2_FG2R+1  ; Address where to draw the dot
        CLRA
...
* Fill the pixels on the left side of the line then draw bytes
        LDA     LineColour      ; Get the line colour
        BNE     LineHorizontalSET_FG2R     ; If it's not zero then draw white line
...
* Do the RESET version of a Horizontal line
        ...
        LDU     #LineDrawSetTableStart_FG2R
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_FG2R       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
BOX_FG2R:
        LDA     LineColour      ; Get the line colour
        BNE     DoBOX_FG2R      ; If it's not zero then draw white Box
PRESETBOX_FG2R:
        LDD     #RESET_FG2R        ; Self mod address of PRESET routine
        BRA     >
DoBOX_FG2R:
        LDD     #SET_FG2R          ; Self mod address of PSET routine
!       STD     BoxDrawDot0_FG2R+1
        STD     BoxDrawDot1_FG2R+1
        STD     BoxDrawDot2_FG2R+1
        STD     BoxDrawDot3_FG2R+1
...
BoxDrawDot0_FG2R:
!       JSR     SET_FG2R      ; Go draw the pixel on screen
...
BoxFill_FG2R:
        LDA     LineColour      ; Get the line colour
        BNE     DoBoxFill_FG2R  ; If it's not zero then fill a white Box
PRESETBoxFill_FG2R:
        LDD     #RESET_FG2R          ; Self mod address of PSET routine
        BRA     >
DoBoxFill_FG2R:
        LDD     #SET_FG2R          ; Self mod address of PSET routine
!       STD     BoxFillDrawDot0_FG2R+1
...
!       JSR     SET_FG2R           ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_FG2R       ; If <=, then keep looping
