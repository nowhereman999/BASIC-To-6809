...
DrawStart               FCB     96,128        ; y,x co-ordinates of the Draw Start
...
DrawLine4:
        LDA     DrawBlankFlag   ; Get the flag to see if we are drawing a blank line or not
...
NoUpdateStart:
        LDA     FORCOL          ; Get the foreground colour value
        BEQ     >               ; If the colour is black then skip ahead
        JSR     LINE4           ; Draw the white line
        PULS    X               ; Restore the pointer to our draw string
        BRA     DrawMainLoop4   ; go get next command
!       JSR     PRESETLINE4     ; Draw the Black line
        PULS    X               ; Restore the pointer to our draw string
        BRA     DrawMainLoop4   ; go get next command
