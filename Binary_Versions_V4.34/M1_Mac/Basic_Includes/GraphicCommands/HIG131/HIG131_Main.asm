; Graphics commands for screen mode HIG131
; Resolution of 256x200x2 colours
ScreenWidth_HIG131    EQU 256
ScreenHeight_HIG131   EQU 200
BytesPerRow_HIG131    EQU ScreenWidth_HIG131/8
Screen_Size_HIG131    EQU $1900
;
; SET a dot on screen
; Enter with:
; A = y coordinate
...
DoSet_HIG131:
        ...
        BEQ     RESET_HIG131      ; If it's not zero then draw white dot
SET_HIG131:
        ...
        LDU     #PixelTable_HIG131 ; U points to the table of pixel values
...
RESET_HIG131:
        ...
        LDB     1,S             ; Get B back which has the pixel number
        ANDB    #%00000111      ; B = Pixel value (0 to 7)
        LDU     #PixelTableClear_HIG131 ; U points to the table of pixel values
        LDA     ,X              ; Get existing pixels in this byte from the screen
        ANDA    B,U             ; AND original Byte value with the pixel the user wants to cleared
        STA     ,X              ; Write new byte to the screen
        PULS    D,PC            ; Restore D,X & U and return

PixelTableClear_HIG131:
...
GCLS_HIG131:
        LDX     BEGGRP
        LEAU    Screen_Size_HIG131,X ; Screen Size
        STU     GCLSComp_HIG131+1 ; Self mod end address
        TSTB
        BEQ     >
        LDB     #$FF
!       TFR     B,A     ; A = B
