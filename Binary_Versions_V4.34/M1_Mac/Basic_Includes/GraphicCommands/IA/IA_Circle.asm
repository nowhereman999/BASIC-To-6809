CircleScale_IA     EQU 170     ; 128 too wide, 96 wider, 192 pretty good, 225 too tall
...
; decision= decision+ 4 * (x - y) + 10
        ...
        ADDD    decision
        STD     decision
        BRA     CircleLoop_IA          ; Loop until x <= y
; else:
...
; Plot (x + x_Center, y + y_Center)
        LDA     y0+1
        LDB     #CircleScale_IA    ; seems to scale better
        MUL                         ; Multiply A * B
        ADDA    y_Center+1
        LDB     x0+1
        ADDB    x_Center+1
        BSR     DoSetYXCheck_IA    ; Plot the pixel
...
DoSetYXCheck_IA:
    	...
    	BPL     >
        CLRA
!       CMPA    #ScreenHeight_IA-1
        BLE     >
        LDA     #ScreenHeight_IA-1
!       TSTB
        BPL     >
        CLRB
!       CMPB    #ScreenWidth_IA-1
        BLE     >
        LDB     #ScreenWidth_IA-1
!       JMP     SET_IA