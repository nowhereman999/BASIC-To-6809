...
!       CMPD    #BytesPerRow_DMAGraphic-1 ; Check if it's beyond the right side of the screen
        BLE     >               ; Skip ahead if good
        LDD     #BytesPerRow_DMAGraphic-1 ; Make it the right most column
!       STD     x0              ; Save the x value

        LDD     y0              ; Get the y value in D
        BPL     >               ; Skip ahead if positive
        LDD     #$0000          ; Otherwise, make it zero
!       CMPD    #ScreenHeight_DMAGraphic-FontHeight          ; Check if it's beyond the bottom of the screen
        BLE     >               ; Skip ahead if good
        LDD     #ScreenHeight_DMAGraphic-FontHeight          ; Make it the bottom row

!       LDA     #BytesPerRow_DMAGraphic ; A= bytes per row
