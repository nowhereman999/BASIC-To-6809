CircleScale_HIG138    EQU 230   ; 64 way too wide, 196 still too wide
* Circle command
* Enter with:
* x Center is at 4,S
* y Center is at 3,S
...
; decision= decision+ 4 * (x - y) + 10
        ...
        ADDD    decision
        STD     decision
        BRA     CircleLoop_HIG138          ; Loop until x <= y
; else:
...
; Plot (x + x_Center, y + y_Center)
        LDA     y0+1
        LDB     #CircleScale_HIG138 ; seems to scale better
        MUL                     ; Multiply A * B
        ADDA    y_Center+1
        LDB     x0+1
        ADDB    x_Center+1
        BSR     DoSetYXCheck_HIG138        ; Plot the pixel
; Plot (-x + x_Center, y + y_Center)
        LDA     y0+1
        LDB     #CircleScale_HIG138 ; seems to scale better
        MUL                     ; Multiply A * B
