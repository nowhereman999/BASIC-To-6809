; Graphics commands for screen mode HIG130
; Resolution of 256x192x2 colours
ScreenWidth_HIG130    EQU 256
ScreenHeight_HIG130   EQU 192
BytesPerRow_HIG130    EQU ScreenWidth_HIG130/8
Screen_Size_HIG130    EQU $1800
;
; SET a dot on screen
; Enter with:
; A = y coordinate
...
;
; x location is 0 to 254 (32 bytes) per row
...
DoSet_HIG130:
        ...
        BEQ     RESET_HIG130      ; If it's not zero then draw white dot
SET_HIG130:
        ...
        LDU     #PixelTable_HIG130 ; U points to the table of pixel values
...
RESET_HIG130:
        ...
        LDB     1,S             ; Get B back which has the pixel number
        ANDB    #%00000111      ; B = Pixel value (0 to 7)
        LDU     #PixelTableClear_HIG130 ; U points to the table of pixel values
        LDA     ,X              ; Get existing pixels in this byte from the screen
        ANDA    B,U             ; AND original Byte value with the pixel the user wants to cleared
        STA     ,X              ; Write new byte to the screen
        PULS    D,PC            ; Restore D,X & U and return

PixelTableClear_HIG130:
...
GCLS_HIG130:
        LDX     BEGGRP
        LEAU    Screen_Size_HIG130,X ; Screen Size
        STU     GCLSComp_HIG130+1 ; Self mod end address
        TSTB
        BEQ     >
        LDB     #$FF
!       TFR     B,A     ; A = B
