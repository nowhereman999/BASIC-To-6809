...
; decision= decision+ 4 * (x - y) + 10
        ...
        ADDD    decision
        STD     decision
        BRA     CircleLoop_HIG100          ; Loop until x <= y
; else:
...
; Plot (x + x_Center, y + y_Center)
        LDB     x0+1
        LDA     #CircleScale_HIG100 ; seems to scale better
        MUL                     ; Multiply A * B
        TFR     A,B
        ADDB    x_Center+1
        LDA     y0+1
        ADDA    y_Center+1
        JSR     DoSetYXCheck_HIG100        ; Plot the pixel
; Plot (-x + x_Center, y + y_Center)
        LDB     x0+1
        LDA     #CircleScale_HIG100 ; seems to scale better
        MUL                     ; Multiply A * B
        ...
        LDA     y0+1
        ADDA    y_Center+1
        BSR     DoSetYXCheck_HIG100        ; Plot the pixel
; Plot (x + x_Center, -y + y_Center)
        LDB     x0+1
        LDA     #CircleScale_HIG100 ; seems to scale better
        MUL                     ; Multiply A * B
...
DoSetYXCheck_HIG100:
!           CMPA    #ScreenHeight_HIG100-1
            BLS     @GoodA
; If we get here the value is not in screen's range
			CMPA	#255-((255-ScreenHeight_HIG100-1)/2) 
			BLS		>			; If we are <= the midpoint beyond the screen size then use max value
			CLRA				; otherwise make it zero
			BRA		@GoodA
!           LDA     #ScreenHeight_HIG100-1
@GoodA      CMPB    #ScreenWidth_HIG100-1
            BLS     @GoodB
; If we get here the value is not in screen's range
			CMPB	#255-((255-ScreenWidth_HIG100-1)/2) 
			BLS		>			; If we are <= the midpoint beyond the screen size then use max value
			CLRB				; otherwise make it zero
			BRA		@GoodB
!           LDB     #ScreenWidth_HIG100-1
@GoodB      JMP     SET_HIG100    ; go set the pixel
;
