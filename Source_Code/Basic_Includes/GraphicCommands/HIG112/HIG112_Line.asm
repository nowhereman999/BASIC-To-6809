...
LINE_HIG112:
        ...
        BNE     DoLINE_HIG112     ; If it's not zero then draw white line
RESETLINE_HIG112:
        LDX     #RESET_HIG112     ; Self mod address of RESET routine
        BRA     >
DoLINE_HIG112:
        LDX     #SET_HIG112       ; Self mod address of SET routine
!       STX     LineDrawDot2_HIG112+1  ; Address where to draw the dot

; Test for a horizontal line
        CMPB    startY+1        ; Compare the starting y coordinate with the ending y coordinate
        LBNE    LineNotHorizontal_HIG112  ; If they aren't the same then go draw a line normally
; Get the number of bytes between pixels
        ...
        LBEQ    LineDrawDot1_HIG112    ; If zero then go PSET one single pixel
...
!       LDB     endX+1
        ...
        LBLS    LineNotHorizontal_HIG112  ; If the size is <= 16 then go draw the line normally
...
* Fill the pixels on the left side of the line then draw bytes
        LDA     LineColour      ; Get the line colour
        BNE     LineHorizontalSET_HIG112     ; If it's not zero then draw white line
...
* Do the RESET version of a Horizontal line
        ...
        LDU     #LineDrawSetTableStart_HIG112
...
* Draw the pixels on the right side of the line
        ...
        LDU     #LineDrawSetTableEnd_HIG112
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_HIG112       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
BOX_HIG112:
        LDA     LineColour      ; Get the line colour
        BNE     DoBOX_HIG112      ; If it's not zero then draw white Box
PRESETBOX_HIG112:
        LDD     #RESET_HIG112        ; Self mod address of PRESET routine
        BRA     >
DoBOX_HIG112:
        LDD     #SET_HIG112          ; Self mod address of PSET routine
!       STD     BoxDrawDot0_HIG112+1
        STD     BoxDrawDot1_HIG112+1
        STD     BoxDrawDot2_HIG112+1
        STD     BoxDrawDot3_HIG112+1
        PULS    Y               ; Y = return address
        LDD     7,S
...
BoxDrawDot0_HIG112:
!       JSR     SET_HIG112      ; Go draw the pixel on screen
...
BoxFill_HIG112:
        LDA     LineColour      ; Get the line colour
        BNE     DoBoxFill_HIG112  ; If it's not zero then fill a white Box
PRESETBoxFill_HIG112:
        LDD     #RESET_HIG112          ; Self mod address of PSET routine
        BRA     >
DoBoxFill_HIG112:
        LDD     #SET_HIG112          ; Self mod address of PSET routine
!       STD     BoxFillDrawDot0_HIG112+1
        PULS    Y               ; Y = return address
        LDD     7,S
        STA     BoxStartY       ; Store in BoxStartY
        STB     BoxStartX       ; Store in BoxStartX
...
!       JSR     SET_HIG112           ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_HIG112       ; If <=, then keep looping
