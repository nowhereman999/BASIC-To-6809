...
LINE_DMAGraphic:
        ...
        BNE     DoLINE_DMAGraphic     ; If it's not zero then draw white line
RESETLINE_DMAGraphic:
        LDX     #RESET_DMAGraphic     ; Self mod address of PSET routine
        BRA     >
DoLINE_DMAGraphic:
        LDX     #SET_DMAGraphic       ; Self mod address of PSET routine
!       STX     LineDrawDot2_DMAGraphic+1  ; Address where to draw the dot
        CLRA
...
; Draw the bits at the beginning that aren't a complete byte
        ...
        LDB     #BytesPerRow_DMAGraphic ; MUL A by the number of bytes per row
        MUL      
...
* Fill the pixels on the left side of the line then draw bytes
        LDA     LineColour      ; Get the line colour
        BNE     LineHorizontalSET_DMAGraphic     ; If it's not zero then draw white line
...
* Do the RESET version of a Horizontal line
        ...
        LDU     #LineDrawSetTableStart_DMAGraphic
...
* Draw the pixels on the right side of the line
        ...
        LDU     #LineDrawSetTableEnd_DMAGraphic
...
LineDrawSetTableStart_DMAGraphic:
        ...
        FCB     %00000000       ; 
LineDrawSetTableEnd_DMAGraphic:

; PSET(startX , startY , 1)
LineDrawDot1_DMAGraphic:
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_DMAGraphic       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
; GOTO LINELoop
        BRA     LINELoop_DMAGraphic       ; GOTO 10
...
BOX_DMAGraphic:
        LDA     LineColour      ; Get the line colour
        BNE     DoBOX_DMAGraphic      ; If it's not zero then draw white Box
PRESETBOX_DMAGraphic:
        LDD     #RESET_DMAGraphic        ; Self mod address of PRESET routine
        BRA     >
DoBOX_DMAGraphic:
        LDD     #SET_DMAGraphic          ; Self mod address of PSET routine
!       STD     BoxDrawDot0_DMAGraphic+1
        STD     BoxDrawDot1_DMAGraphic+1
        STD     BoxDrawDot2_DMAGraphic+1
        STD     BoxDrawDot3_DMAGraphic+1
...
BoxDrawDot0_DMAGraphic:
!       JSR     SET_DMAGraphic      ; Go draw the pixel on screen
...
BoxFill_DMAGraphic:
        LDA     LineColour      ; Get the line colour
        BNE     DoBoxFill_DMAGraphic  ; If it's not zero then fill a white Box
PRESETBoxFill_DMAGraphic:
        LDD     #RESET_DMAGraphic          ; Self mod address of PSET routine
        BRA     >
DoBoxFill_DMAGraphic:
        LDD     #SET_DMAGraphic          ; Self mod address of PSET routine
!       STD     BoxFillDrawDot0_DMAGraphic+1
...
BFillLoop_DMAGraphic:
        LDB     BoxStartX       ; B = BoxStartX
BoxFillDrawDot0_DMAGraphic:
!       JSR     SET_DMAGraphic           ; Go draw the pixel on screen
        INCB                    ; Increment the x coordinate
        CMPB    BoxEndX         ; Compare the x coordinate with the end x coordinate
        BNE     <               ; If <>, then keep looping
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_DMAGraphic       ; If <=, then keep looping
