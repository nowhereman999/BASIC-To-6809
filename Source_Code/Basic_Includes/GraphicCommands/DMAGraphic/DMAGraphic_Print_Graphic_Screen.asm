...
PRINT_D_Graphics_Screen_DMAGraphic:
...
@PRINT_D1_Graphics_Screen:
        JSR     AtoGraphics_Screen_DMAGraphic
...
!       LDA     _Var_PF00
        BEQ     >
        ADDA    #$30
        BSR     AtoGraphics_Screen_DMAGraphic
...
!       LDA     _Var_PF04
        ...
        BSR     AtoGraphics_Screen_DMAGraphic
        LDA     #$20        ; Blank
        BRA     AtoGraphics_Screen_DMAGraphic ; Print A on the screen and return, Done
@Print1000sGScreen:
        LDA     _Var_PF01
        ADDA    #$30
        BSR     AtoGraphics_Screen_DMAGraphic
...
!       LDU   BEGGRP    ; U = screen start
        LEAU  Screen_Size_DMAGraphic-(FontHeight-1)*BytesPerRow_DMAGraphic,U      ; U = screen End - 7 rows
        PSHS  U         ; Save the value on the stack
        CMPX  ,S++      ; Compare X with the value on the stack, fix the stack
        BLO   >
        LDX   BEGGRP    ; X = Screen Start
        LEAX  Screen_Size_DMAGraphic-BytesPerRow_DMAGraphic*FontHeight,X  ; X = Screen Start + screen size - 1 row = bottom of the screen
        STX   GraphicCURPOS
!       CMPA  #$0D      ; Enter Key?
...
!       BSR   @GraphicTextDrawAatX  ; Go Draw character A on screen at X
        ...
        TFR   X,B       ; B = X (LSB)
        BITB  #BytesPerRow_DMAGraphic-1      ; Is it the start of a new row?
        BNE   <         ; if not keep printing a blank
        LEAX  (FontHeight-1)*BytesPerRow_DMAGraphic,X    ; Point at the start of the next row
        LEAU  (FontHeight-1)*BytesPerRow_DMAGraphic,U    ; U = screen End
        PSHS  U         ; Save the value on the stack
        CMPX  ,S++      ; Compare X with the value on the stack, fix the stack
...
!       CMPB    #96             ; Is it a printable character?
        BLO     >               ; Yes, skip ahead
        CLRB                    ; Otherwise, make it a space
!       LDX     #CharJumpTable_DMAGraphic ; Get the address of the character jump table
