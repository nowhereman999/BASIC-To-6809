...
LINE_HIG156:
        ...
        BNE     DoLINE_HIG156     ; If it's not zero then draw white line
RESETLINE_HIG156:
        LDU     #RESET_HIG156     ; Self mod address of RESET routine
        BRA     >
DoLINE_HIG156:
        LDU     #SET_HIG156       ; Self mod address of SET routine
!       STU     LineDrawDot2_HIG156+1  ; Address where to draw the dot

; Test for a horizontal line
        CMPB    startY+1        ; Compare the starting y coordinate with the ending y coordinate
        LBNE    LineNotHorizontal_HIG156  ; If they aren't the same then go draw a line normally
; Get the number of bytes between pixels
        ...
        LBEQ    LineDrawDot1_HIG156    ; If zero then go SET one single pixel
...
!       LDD     endX
        ...
        LBLS    LineNotHorizontal_HIG156  ; If the size is <= 16 then go draw the line normally
...
* Fill the pixels on the left side of the line then draw bytes
        LDA     LineColour      ; Get the line colour
        BNE     LineHorizontalSET_HIG156     ; If it's not zero then draw white line
...
* Do the RESET version of a Horizontal line
        ...
        LDU     #LineDrawSetTableStart_HIG156
...
* Draw the pixels on the right side of the line
        ...
        LDU     #LineDrawSetTableEnd_HIG156
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_HIG156       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
BOX_HIG156:
        LDA     LineColour      ; Get the line colour
        BNE     DoBOX_HIG156      ; If it's not zero then draw white Box
PRESETBOX_HIG156:
        LDD     #RESET_HIG156        ; Self mod address of PRESET routine
        BRA     >
DoBOX_HIG156:
        LDD     #SET_HIG156          ; Self mod address of PSET routine
!       STD     BoxDrawDot0_HIG156+1
        STD     BoxDrawDot1_HIG156+1
        STD     BoxDrawDot2_HIG156+1
        STD     BoxDrawDot3_HIG156+1
...
BoxDrawDot0_HIG156:
!       JSR     SET_HIG156      ; Go draw the pixel on screen
...
BoxFill_HIG156:
        LDA     LineColour      ; Get the line colour
        BNE     DoBoxFill_HIG156  ; If it's not zero then fill a white Box
PRESETBoxFill_HIG156:
        LDD     #RESET_HIG156          ; Self mod address of PSET routine
        BRA     >
DoBoxFill_HIG156:
        LDD     #SET_HIG156          ; Self mod address of PSET routine
!       STD     BoxFillDrawDot0_HIG156+1
...
!       JSR     SET_HIG156      ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_HIG156 ; If <=, then keep looping
