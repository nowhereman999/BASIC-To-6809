...
; Test for a horizontal line
        ...
        LBNE    LineNotHorizontal_FG6C  ; If they aren't the same then go draw a line normally
...
!       PSHS    A               ; save the pixel pattern on the stack
        ...
        LDU     #LineDrawSetTableStart_FG6C
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_FG6C       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
!       JSR     SET_FG6C      ; Go draw the pixel on screen
...
!       JSR     SET_FG6C           ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_FG6C       ; If <=, then keep looping
