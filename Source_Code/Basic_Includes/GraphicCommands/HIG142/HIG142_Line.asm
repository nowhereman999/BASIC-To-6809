...
ReadyToDoHorizontalLine_HIG142:
...
!       PSHS    A               ; save the pixel pattern on the stack
        ...
        LDU     #LineDrawSetTableStart_HIG142
...
* Draw the pixels on the right side of the line
        LDU     #LineDrawSetTableEnd_HIG142
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_HIG142       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
BoxDrawDot0_HIG142:
!       JSR     SET_HIG142      ; Go draw the pixel on screen
...
!       JSR     SET_HIG142      ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_HIG142 ; If <=, then keep looping
