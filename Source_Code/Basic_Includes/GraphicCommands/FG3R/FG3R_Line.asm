...
LINE_FG3R:
        ...
        BNE     DoLINE_FG3R     ; If it's not zero then draw white line
RESETLINE_FG3R:
        LDX     #RESET_FG3R     ; Self mod address of PSET routine
        BRA     >
DoLINE_FG3R:
        LDX     #SET_FG3R       ; Self mod address of PSET routine
!       STX     LineDrawDot2_FG3R+1  ; Address where to draw the dot
        CLRA
...
; Test for a horizontal line
        ...
        LBNE    LineNotHorizontal_FG3R  ; If they aren't the same then go draw a line normally
; Get the number of bytes between pixels
        ...
        LBEQ    LineDrawDot1_FG3R    ; If A is zero then go PSET one single pixel
...
!       LDB     endX+1
        ...
        LBLS    LineNotHorizontal_FG3R  ; If the size is <= 16 then go draw the line normally
...
* Fill the pixels on the left side of the line then draw bytes
        LDA     LineColour      ; Get the line colour
        BNE     LineHorizontalSET_FG3R     ; If it's not zero then draw white line
...
* Do the RESET version of a Horizontal line
        ...
        LDU     #LineDrawSetTableStart_FG3R
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_FG3R       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
BOX_FG3R:
        LDA     LineColour      ; Get the line colour
        BNE     DoBOX_FG3R      ; If it's not zero then draw white Box
PRESETBOX_FG3R:
        LDD     #RESET_FG3R        ; Self mod address of PRESET routine
        BRA     >
DoBOX_FG3R:
        LDD     #SET_FG3R          ; Self mod address of PSET routine
!       STD     BoxDrawDot0_FG3R+1
        STD     BoxDrawDot1_FG3R+1
        STD     BoxDrawDot2_FG3R+1
        STD     BoxDrawDot3_FG3R+1
...
BoxDrawDot0_FG3R:
!       JSR     SET_FG3R      ; Go draw the pixel on screen
...
BoxFill_FG3R:
        LDA     LineColour      ; Get the line colour
        BNE     DoBoxFill_FG3R  ; If it's not zero then fill a white Box
PRESETBoxFill_FG3R:
        LDD     #RESET_FG3R          ; Self mod address of PSET routine
        BRA     >
DoBoxFill_FG3R:
        LDD     #SET_FG3R          ; Self mod address of PSET routine
!       STD     BoxFillDrawDot0_FG3R+1
...
!       JSR     SET_FG3R           ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_FG3R       ; If <=, then keep looping
