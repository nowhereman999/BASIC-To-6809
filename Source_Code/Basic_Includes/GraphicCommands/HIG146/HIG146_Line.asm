...
; Test for a horizontal line
        ...
        LBNE    LineNotHorizontal_HIG146  ; If they aren't the same then go draw a line normally
; Get the number of bytes between pixels
        ...
        BEQ     LineDrawDot1_HIG146    ; If zero then go SET one single pixel
        LDU     startX          ; Otherwise flip the startx and endx coordinates
        STU     endX            ;
        STX     startX          ;

ReadyToDoHorizontalLine_HIG146:
* See if we have more than 4 pixels to draw
!       LDD     endX
        SUBD    startX          ;
        CMPD    #4
        BLS     LineNotHorizontal_HIG146  ; If the size is <= 4 then go draw the line normally
        TFR     D,X
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_HIG146       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
BoxDrawDot0_HIG146:
!       JSR     BoxSet_HIG146   ; Go draw the pixel on screen
...
!       JSR     BoxSet_HIG146   ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_HIG146 ; If <=, then keep looping
