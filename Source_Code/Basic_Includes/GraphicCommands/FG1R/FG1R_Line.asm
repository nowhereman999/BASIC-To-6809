...
LINE_FG1R:
        ...
        BNE     DoLINE_FG1R     ; If it's not zero then draw white line
RESETLINE_FG1R:
        LDX     #RESET_FG1R     ; Self mod address of PSET routine
        BRA     >
DoLINE_FG1R:
        LDX     #SET_FG1R       ; Self mod address of PSET routine
!       STX     LineDrawDot2_FG1R+1  ; Address where to draw the dot
        CLRA
...
; Test for a horizontal line
        ...
        LBNE    LineNotHorizontal_FG1R  ; If they aren't the same then go draw a line normally
; Get the number of bytes between pixels
        ...
        LBEQ    LineDrawDot1_FG1R    ; If A is zero then go PSET one single pixel
...
!       LDB     endX+1
        ...
        LBLS    LineNotHorizontal_FG1R  ; If the size is <= 16 then go draw the line normally
; 22 / 8 = 2
; 2 - 1 = 1
...
* Fill the pixels on the left side of the line then draw bytes
        LDA     LineColour      ; Get the line colour
        BNE     LineHorizontalSET_FG1R     ; If it's not zero then draw white line
...
* Do the RESET version of a Horizontal line
        ...
        LDU     #LineDrawSetTableStart_FG1R
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_FG1R       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
BOX_FG1R:
        LDA     LineColour      ; Get the line colour
        BNE     DoBOX_FG1R      ; If it's not zero then draw white Box
PRESETBOX_FG1R:
        LDD     #RESET_FG1R        ; Self mod address of PRESET routine
        BRA     >
DoBOX_FG1R:
        LDD     #SET_FG1R          ; Self mod address of PSET routine
!       STD     BoxDrawDot0_FG1R+1
        STD     BoxDrawDot1_FG1R+1
        STD     BoxDrawDot2_FG1R+1
        STD     BoxDrawDot3_FG1R+1
...
BoxDrawDot0_FG1R:
!       JSR     SET_FG1R      ; Go draw the pixel on screen
...
BoxFill_FG1R:
        LDA     LineColour      ; Get the line colour
        BNE     DoBoxFill_FG1R  ; If it's not zero then fill a white Box
PRESETBoxFill_FG1R:
        LDD     #RESET_FG1R          ; Self mod address of PSET routine
        BRA     >
DoBoxFill_FG1R:
        LDD     #SET_FG1R          ; Self mod address of PSET routine
!       STD     BoxFillDrawDot0_FG1R+1
...
!       JSR     SET_FG1R           ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_FG1R       ; If <=, then keep looping
