...
PaintWhileLoop1_HIG111:           ; Start of WHILE/WEND loop
        CMPS    PaintStack      ; Compare the stack pointer with the stack address
        LBEQ    PaintWend_01_HIG111    ; If Equal, then jump to the end of the WHILE/WEND loop
...
PaintWhileLoop2_HIG111                ; Start of WHILE/WEND loop
        LDX     currentX 
        BMI     PaintWend_02_HIG111   ; If the result is a false then goto the end of the WHILE/WEND loop
        LDA     PaintY+1
        LDB     currentX+1
        JSR     POINT_HIG111          ; Return with the colour value on screen in B
        CMPB    SourceColour        ; Compare the existing colour with the Border Colour
        BNE     PaintWend_02_HIG111   ; If they are not equal then goto the end of the WHILE/WEND loop (paint the line to the right)
; currentX = currentX - 1
...
; WEND
        BRA    PaintWhileLoop2_HIG111 ; Goto the start of this WHILE/WEND loop

PaintWend_02_HIG111                   ; End of WHILE/WEND loop
; currentX = currentX + 1
        LDD     currentX     
        ADDD    #1     
        STD     currentX        ; Save Numeric variable
...
PaintWhileLoop3_HIG111                ; Start of WHILE/WEND loop
        LDX     currentX
        CMPX    #ScreenWidth_HIG111   ; Check if X is less than ScreenWidth
        LBHS    PaintWend_03_HIG111   ; If the result is a false then goto the end of the WHILE/WEND loop

        LDA     PaintY+1
        LDB     currentX+1
        JSR     POINT_HIG111     ; Return with the colour value of the PPoint on screen in B
        CMPB    SourceColour       ; Compare the old color with the new color
        LBNE    PaintWend_03_HIG111  ; If they are not equal then goto the end of the WHILE/WEND loop

; SET(currentX , y , FillColour)
        LDA     PaintY+1
        LDB     currentX+1
        JSR     SET_HIG111       ; Plot the pixel

; IF spanAbove = false AND y > 0 AND PPOINT(currentX , y -1) = SourceColour THEN
        LDX     spanAbove       ; Get the spanAbove value
        BNE     PaintELSE_02_HIG111        ; If result is <> zero = FALSE then jump to ELSE/Next line

        LDX     PaintY          ; Get the Y co-ordinate
        BLE     PaintELSE_02_HIG111        ; If the result is <= 0 then jump to ELSE/Next line
        ...
        JSR     POINT_HIG111      ; Return with the colour value of the PPoint on screen in B
        CMPB    SourceColour        ; Compare the old color with the new color
        BNE     PaintELSE_02_HIG111        ; If they are not equal then goto the end of the WHILE/WEND loop
...
; spanAbove = true
        LDD     #TRUE     
        STD     spanAbove   ; Save Numeric variable
        BRA     PaintIFDone_02_HIG111    ; Jump to END IF line
; ELSE
PaintELSE_02_HIG111                              ; If result is zero = FALSE then jump to ELSE/Next line
; IF spanAbove = true AND y > 0 AND PPOINT(currentX , y -1) < > SourceColour THEN
        LDX     spanAbove       ; Get the spanAbove value
        BEQ     PaintIFDone_03_HIG111      ; If result is zero = FALSE then jump to END IF line

        LDX     PaintY          ; Get the Y co-ordinate
        BLE     PaintIFDone_03_HIG111      ; If result is <= 0 then jump to END IF line
        ...
        JSR     POINT_HIG111      ; Return with the colour value of the PPoint on screen in B
        CMPB    SourceColour        ; Compare the old color with the new color
        BEQ     PaintIFDone_03_HIG111      ; If result is = then jump to END IF line

; spanAbove = false
        LDD     #FALSE    
        STD     spanAbove   ; Save Numeric variable
; END IF
PaintIFDone_03_HIG111                              ; END IF line
; END IF
PaintIFDone_02_HIG111                              ; END IF line
; IF spanBelow = false AND y < SCREEN_HEIGHT -1 AND PPOINT(currentX , y + 1) = SourceColour THEN
        LDX     spanBelow   
        BNE     PaintELSE_04_HIG111        ; If result is <> zero then jump to ELSE/Next line

        LDX     #ScreenHeight_HIG111
        LEAX    -1,X            ; Decrement the screen height comaprison value
        CMPX    PaintY          ; Check if X is with the y co-ordinate
        BLS     PaintELSE_04_HIG111        ; If the result is a false then jump to ELSE/Next line
        ...
        JSR     POINT_HIG111      ; Return with the colour value of the PPoint on screen in B
        CMPB    SourceColour        ; Compare the old color with the new color
        BNE     PaintELSE_04_HIG111        ; If the result is <> then jump to ELSE/Next line
...
; spanBelow = true
        ...
        STD     spanBelow   ; Save Numeric variable
; ELSE
        BRA     PaintIFDone_04_HIG111    ; Jump to END IF line
PaintELSE_04_HIG111                              ; If result is zero = FALSE then jump to ELSE/Next line
; IF spanBelow = true AND y < SCREEN_HEIGHT -1 AND PPOINT(currentX , y + 1) < > SourceColour THEN
        LDX     spanBelow       ; Get the spanBelow value
        BEQ     PaintIFDone_05_HIG111      ; If result is zero = FALSE then jump to END IF line

        LDX     #ScreenHeight_HIG111
        LEAX    -1,X            ; Decrement the screen height comaprison value
        CMPX    PaintY          ; Check if X is with the y co-ordinate
        BLS     PaintIFDone_05_HIG111      ; If result is >= then jump to END IF line
        ...
        JSR     POINT_HIG111      ; Return with the colour value of the PPoint on screen in B
        CMPB    SourceColour        ; Compare the old color with the new color
        BEQ     PaintIFDone_05_HIG111      ; If the result is = then jump to ELSE/Next line

; spanBelow = false
        LDD     #FALSE    
        STD     spanBelow   ; Save Numeric variable
; END IF
PaintIFDone_05_HIG111                              ; END IF line
; END IF
PaintIFDone_04_HIG111                              ; END IF line
; currentX = currentX + 1
        LDD     currentX 
        ADDD    #1
        STD     currentX ; Save Numeric variable
; WEND
        LBRA    PaintWhileLoop3_HIG111  ; Goto the start of this WHILE/WEND loop
PaintWend_03_HIG111                       ; End of WHILE/WEND loop
; WEND
        LBRA    PaintWhileLoop1_HIG111  ; Goto the start of this WHILE/WEND loop
PaintWend_01_HIG111                       ; End of WHILE/WEND loop
