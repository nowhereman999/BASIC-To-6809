; Graphics commands for screen mode HIG149
; Resolution of 512x200x2 colours
ScreenWidth_HIG149    EQU 512
ScreenHeight_HIG149   EQU 200
BytesPerRow_HIG149    EQU ScreenWidth_HIG149/8
Screen_Size_HIG149    EQU $3200
;
; SET a dot on screen
; Enter with:
; A = y coordinate
...
DoSet_HIG149:
        ...
        BEQ     RESET_HIG149      ; If it's not zero then draw white dot
SET_HIG149:
        ...
        LDU     #PixelTable_HIG149 ; U points to the table of pixel values
...
RESET_HIG149:
        ...
        LDB     2,S             ; Get B back which has the pixel number
        ANDB    #%00000111      ; B = Pixel value (0 to 7)
        LDU     #PixelTableClear_HIG149 ; U points to the table of pixel values
        LDA     ,X              ; Get existing pixels in this byte from the screen
        ANDA    B,U             ; AND original Byte value with the pixel the user wants to cleared
        STA     ,X              ; Write new byte to the screen
        PULS    A,X,PC            ; Restore D,X & U and return

PixelTableClear_HIG149:
...
GCLS_HIG149:
        LDX     BEGGRP
        LEAU    Screen_Size_HIG149,X ; Screen Size
        STU     GCLSComp_HIG149+1 ; Self mod end address
        TSTB
        BEQ     >
        LDB     #$FF
!       TFR     B,A     ; A = B
