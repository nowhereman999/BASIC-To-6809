...
!       CMPD    #BytesPerRow_SG12-1 ; Check if it's beyond the right side of the screen
        BLE     >               ; Skip ahead if good
        LDD     #BytesPerRow_SG12-1 ; Make it the right most column
!       STD     x0              ; Save the x value

        LDD     y0              ; Get the y value in D
        BPL     >               ; Skip ahead if positive
        LDD     #$0000          ; Otherwise, make it zero
!       CMPD    #ScreenHeight_SG12/2-1          ; Check if it's beyond the bottom of the screen
        BLE     >               ; Skip ahead if good
        LDD     #ScreenHeight_SG12/2-1          ; Make it the bottom row

!       LSLB                    ; Multiply by 2
        PSHS    B               ; Save the y value on the stack
        LSLB                    ; Multiply by 4
        ADDB    ,S+
        LDA     #BytesPerRow_SG12 ; A= bytes per row
