...
!       LDA     _Var_PF04
        ...
        BSR     AtoGraphics_Screen_SG12
        LDA     #$20        ; Blank
        BRA     AtoGraphics_Screen_SG12 ; Print A on the screen and return, Done
@Print1000sGScreen:
        LDA     _Var_PF01
        ADDA    #$30
        BSR     AtoGraphics_Screen_SG12
...
@Print1sGScreen:
        ...
        BSR     AtoGraphics_Screen_SG12
        LDA     #$20        ; Blank
        BRA     AtoGraphics_Screen_SG12 ; Print A on the screen and return

AtoGraphics_Screen_SG12:
...
!       LDU   BEGGRP    ; U = screen start
        LEAU  Screen_Size_SG12-(FontHeight-1)*BytesPerRow_SG12,U      ; U = screen End - 7 rows
        PSHS  U         ; Save the value on the stack
        CMPX  ,S++      ; Compare X with the value on the stack, fix the stack
        BLO   >
        LDX   BEGGRP    ; X = Screen Start
        LEAX  Screen_Size_SG12-BytesPerRow_SG12*FontHeight,X  ; X = Screen Start + screen size - 1 row = bottom of the screen
        STX   GraphicCURPOS
!       CMPA  #$0D      ; Enter Key?
...
!       CMPD  #BytesPerRow_SG12          ; Is D less than BytesPerRow_SG12?
        BLO   >             ; If yes, we're done
        SUBD  #BytesPerRow_SG12          ; Subtract one full row
        BRA   <             ; Repeat until D < BytesPerRow_SG12
; B now contains the column number
!       SUBB   #BytesPerRow_SG12      ; B = B - ByttesPerRow_SG12
        LDA   #$80      ; Blank
!       BSR   @GraphicTextDrawAatX  ; Go Draw character A on screen at X
        LEAX  BytesPerChar,X       ; Move to the next position
        INCB
        BNE   <         ; if not keep printing a blank
        LEAX  (FontHeight-1)*BytesPerRow_SG12,X    ; Point at the start of the next row
        LEAU  (FontHeight-1)*BytesPerRow_SG12,U    ; U = screen End
        PSHS  U         ; Save the value on the stack
        CMPX  ,S++      ; Compare X with the value on the stack, fix the stack
...
@GraphicTextDrawAatX:
        ...
        STA     BytesPerRow_SG12,X ; Draw the character
        STA     BytesPerRow_SG12*2,X ; Draw the character
        STA     BytesPerRow_SG12*3,X ; Draw the character
        STA     BytesPerRow_SG12*4,X ; Draw the character
        STA     BytesPerRow_SG12*5,X ; Draw the character
        RTS                     ; Return
