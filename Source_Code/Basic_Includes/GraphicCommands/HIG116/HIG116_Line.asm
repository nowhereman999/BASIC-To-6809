...
; Test for a horizontal line
        ...
        LBNE    LineNotHorizontal_HIG116  ; If they aren't the same then go draw a line normally
...
; Turn pixels into bytes
        ...
        LSRB
        PSHS    B  
        LDB     #BytesPerRow_HIG116 ; MUL A by the number of bytes per row
        MUL      
...
!       PSHS    A               ; save the pixel pattern on the stack
        ...
        LDU     #LineDrawSetTableStart_HIG116
...
* Draw the pixels on the right side of the line
        LDU     #LineDrawSetTableEnd_HIG116
        LDB     endX+1          ; B = ending x coordinate
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_HIG116       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
!       JSR     SET_HIG116      ; Go draw the pixel on screen
...
!       JSR     SET_HIG116           ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_HIG116       ; If <=, then keep looping
