...
; Get the number of bytes between pixels
        ...
        BEQ     LineDrawDot1_HIG137    ; If zero then go SET one single pixel
        LDA     startX+1          ; Otherwise flip the startx and endx coordinates
        LDB     endX+1          ; and store them in the variables
        STA     endX+1          ;
        STB     startX+1        ;

ReadyToDoHorizontalLine_HIG137:
* See if we have more than 4 pixels to draw
!       LDB     endX+1
        SUBB    startX+1        ;
        CMPB    #5
        BLS     LineNotHorizontal_HIG137  ; If the size is <= 4 then go draw the line normally
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_HIG137       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
!       JSR     SET_HIG137           ; Go draw the pixel on screen
...
!       LDB     BoxEndX
        ...
        BLS     Normal_BFillLoop_HIG137  ; If the size is <= 8 then go draw the BoxFill normally
        PSHS    Y                ; Make RTS be the return address
        LDY     #LineReturn_HIG137 ; Make the end of the line routine return below
        LDA     BoxStartX
        STA     startX+1   
        LDA     BoxEndX 
        STA     endX+1
...
!       STB     startY+1         ; Store in startY
        ...
        JMP     ReadyToDoHorizontalLine_HIG137 ; Draw a horizontal line
LineReturn_HIG137:
...
!       JSR     SET_HIG137        ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_HIG137       ; If <=, then keep looping
