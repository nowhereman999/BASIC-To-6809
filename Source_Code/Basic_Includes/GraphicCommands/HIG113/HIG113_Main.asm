; Graphics commands for screen mode HIG113
; Resolution of 128x200x2 colours
ScreenWidth_HIG113    EQU 128
ScreenHeight_HIG113   EQU 200
BytesPerRow_HIG113    EQU ScreenWidth_HIG113/8
Screen_Size_HIG113    EQU $0C80
;
; Do SET
; Enter with:
...
DoSet_HIG113:
        ...
        BEQ     RESET_HIG113    ; If it's not zero then draw white dot
SET_HIG113:
...
!       LSRA
        ...
        LDU     #PixelTable_HIG113 ; U points to the table of pixel values
...
!       LSRA
        ...
        LDB     1,S             ; Get B back which has the pixel number
        ANDB    #%00000111      ; B = Pixel value (0 to 7)
        LDU     #PixelTableClear_HIG113 ; U points to the table of pixel values
        LDA     ,X              ; Get existing pixels in this byte from the screen
        ANDA    B,U             ; AND original Byte value with the pixel the user wants to cleared
        STA     ,X              ; Write new byte to the screen
        PULS    D,PC            ; Restore D,X & U and return

PixelTableClear_HIG113:
...
GCLS_HIG113:
        LDX     BEGGRP
        LEAU    Screen_Size_HIG113,X ; Screen Size
        STU     GCLSComp_HIG113+1 ; Self mod end address
        TSTB
        BEQ     >
        LDB     #$FF
!       TFR     B,A     ; A = B
