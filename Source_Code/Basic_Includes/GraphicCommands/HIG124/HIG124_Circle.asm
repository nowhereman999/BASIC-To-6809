CircleScale_HIG124    EQU 140   ; 64 too skinny, 192 too wide, 128 a little too thin
* Circle command
* Enter with:
* x Center is at 4,S
* y Center is at 3,S
...
; decision= decision+ 4 * (x - y) + 10
        ...
        ADDD    decision
        STD     decision
        BRA     CircleLoop_HIG124          ; Loop until x <= y
; else:
...
; Plot (x + x_Center, y + y_Center)
        LDB     x0+1
        LDA     #CircleScale_HIG124 ; seems to scale better
        MUL                     ; Multiply A * B
        TFR     A,B
        ADDB    x_Center+1
        LDA     y0+1
        ADDA    y_Center+1
        JSR     DoSetYXCheck_HIG124        ; Plot the pixel
; Plot (-x + x_Center, y + y_Center)
        LDB     x0+1
        LDA     #CircleScale_HIG124 ; seems to scale better
        MUL                     ; Multiply A * B
        ...
        LDA     y0+1
        ADDA    y_Center+1
        BSR     DoSetYXCheck_HIG124        ; Plot the pixel
; Plot (x + x_Center, -y + y_Center)
        LDB     x0+1
        LDA     #CircleScale_HIG124 ; seems to scale better
        MUL                     ; Multiply A * B
...
DoSetYXCheck_HIG124:
!       CMPA    #ScreenHeight_HIG124-1
        BLS     @GoodA
; If we get here the value is not in screen's range
	CMPA	#255-((255-ScreenHeight_HIG124-1)/2) 
	BLS	>			; If we are <= the midpoint beyond the screen size then use max value
	CLRA				; otherwise make it zero
	BRA	@GoodA
!       LDA     #ScreenHeight_HIG124-1
@GoodA  CMPB    #ScreenWidth_HIG124-1
        BLS     @GoodB
; If we get here the value is not in screen's range
	CMPB	#255-((255-ScreenWidth_HIG124-1)/2) 
	BLS	>			; If we are <= the midpoint beyond the screen size then use max value
	CLRB				; otherwise make it zero
	BRA	@GoodB
!       LDB     #ScreenWidth_HIG124-1
@GoodB  JMP     SET_HIG124    ; go set the pixel
