; Draw command
        ...
        STD     @Return+1       ; Save the return address below (self mod)
        LEAX    1,S             ; Get the start of the draw command string
        LEAU    1,S             ; Get the start of the draw command string
...
@Skip:  DECB                    ; Decrement the string length
        ...
        LEAX    1,S             ; Point X at the start of the draw command string
DrawMainLoop4:
        LDA     ,X+             ; Get the next character
DrawBack4:
        BNE     >               ; If we get a zero we are done
DrawDone4:
        LDB     ,S+
        CLRA
        LEAS    D,S
@Return:
        JMP     >$FFFF          ; Return from the Draw command
...
DrawB4:
        ...
        LBEQ    DrawMainLoop4   ; If so then we are done with the command
        JMP     DrawBack4       ; Go handle the next character
; Do the Move command
DrawM4:
        CLR     RelativeMoveFlag   ; Clear the flag
; Get the x co-ordinate
        JSR     GetNumberinDrawCommand   ; Get the number from the string
...
;Doing a relative move
        ...
        JSR     DrawLineScale4  ; Go scale the line scaled version is saved in DrawDestinationY(y & x co-ordinates)
!       LDA     AngleValue      ; Get the angle value
        LBNE    DoAngle4        ; If <>zero then we are drawing the line on an angle
        JMP     DrawLine4       ; Go draw the line
AbsoluteMove4:
        STD     DrawDestinationY ; Save the New destination y co-ordinate
        STU     DrawDestinationX  ; Save the New destination x co-ordinate
        JMP     DrawAbsolute    ; Go draw the line at the abolute location
; Do the Up command
DrawU4:
        JSR     GetNumberinDrawCommand   ; Get the number from the string in D
        CMPD    #$0000
...
DrawS4:
        JSR     GetNumberinDrawCommand  ; Get the number from the string in D (actually only need value in B) B)
...
DrawLine4:
        LDA     ScaleFlag
        BEQ     >               ; If Flag is FLASE then skip ahead
        JSR     DrawLineScale4  ; Go scale the line scaled version is saved in DrawDestinationY & X
DrawAbsolute:
