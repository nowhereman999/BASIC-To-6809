...
; Test for a horizontal line
        ...
        LBNE    LineNotHorizontal_HIG151  ; If they aren't the same then go draw a line normally
; Get the number of bytes between pixels
        ...
        LBEQ    LineDrawDot1_HIG151    ; If zero then go SET one single pixel
        LDU     startX          ; Otherwise flip the startx and endx coordinates
        STU     endX            ;
        STX     startX          ;

ReadyToDoHorizontalLine_HIG151:
* See if we have more then 16 pixels to draw
!       LDD     endX
        SUBD    startX          ;
        CMPD    #8             
        LBLS    LineNotHorizontal_HIG151  ; If the size is <= 8 then go draw the line normally
...
!       PSHS    A               ; save the pixel pattern on the stack
        ...
        LDU     #LineDrawSetTableStart_HIG151
...
* Draw the pixels on the right side of the line
        LDU     #LineDrawSetTableEnd_HIG151
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_HIG151       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
BoxDrawDot0_HIG151:
!       JSR     SET_HIG151      ; Go draw the pixel on screen
...
!       JSR     SET_HIG151      ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_HIG151 ; If <=, then keep looping
