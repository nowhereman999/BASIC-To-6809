...
PUT_AND:
        ...
        LDB     B,X
        STB     PutANDFirst1AND0+1   ; Self modify value to keep left bits (below) 
        STB     PutANDFirst2AND1+1   ; Self modify value to keep left bits (below) 
        STB     PutANDFirst3AND1+1   ; Self modify value to keep left bits (below)   
        COMB
        STB     PutANDFirst2AND0+1   ; Self modify value to keep left bits (below)  
        STB     PutANDFirst3AND0+1   ; Self modify value to keep left bits (below)   
...
!       STD     endX            ; Store in endX
        ...
        ANDB    #%00000111      ; B = 0 to 7
        LDB     B,X
        STB     PutANDLast2AND1+1   ; Self modify value to keep left bits (below)   
        STB     PutANDLast3AND1+1   ; Self modify value to keep left bits (below)    
        PSHS    B                   ; Save B
        COMB                        ; Flip the bits
        STB     PutANDLast2AND0+1   ; Self modify value to keep left bits (below) 
        STB     PutANDLast3AND0+1   ; Self modify value to keep left bits (below)  
        PULS    B                   ; Restore B
        ORB     PutANDFirst1AND0+1   ; OR the right bits with the left bits 
        STB     PutANDFirst1AND1+1   ; Self modify value to keep left bits & right bits(below) 
        COMB                        ; Flip the bits
        STB     PutANDFirst1AND0+1   ; Self modify value to keep left bits & right bits(below) 
...
; A = Y co-ordinate and B = X co-ordinate
        ...
        STB     PUTDoAddAND+1   ; Save the BRAnch location below (self mod where to begin adding)
        LDD     #$0000          ; D = 0
PUTDoAddAND:
        BRA     <PUTPositisonZeroAND ; This address will be self modified above to jump to the correct add line below
...
PUTPositisonZeroAND:
        ...
        BHI     PutDoRowAND3Init ; Branch if there are more than 2 bytes to put per row
        DECB
        BNE     PutDoRowAND2    ; Branch if there are 2 bytes to put per row

PutDoRowAND1:
;handle the first column
        LDA     ,U              ; Get buffer value
        ANDA    ,X              ; AND with Screen value
PutANDFirst1AND0:
...
PutANDLast2AND1:
        ...
        ABX                     ; X = X + B (move to the start of the next row)
        BRA     PutDoRowAND2        ; Loop until all rows are copied
; Bytes have been copied to the screen
!       PULS    D,PC            ; Fix the stack and return
...
PutANDLast3AND1:
        ...
        ABX                     ; X = X + B (move to the start of the next row)
        BRA     PutDoRowAND3       ; Loop until all rows are copied
; Bytes have been copied to the screen
!       PULS    D,PC            ; Fix the stack and return
