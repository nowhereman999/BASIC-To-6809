...
LINE_SG4H:
        ...
        STA     C_SG4H           ; Save the colour for the line command to use
        CLRA
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop4_SG4H       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
BoxDrawDot0_SG4H:
!       JSR     DoSetSaveAB_SG4H ; Go draw the pixel on screen
...
!       JSR     DoSetSaveAB_SG4H     ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_SG4H   ; If <=, then keep looping
