CircleScale_EA     EQU 170     ; 128 too wide, 96 wider, 192 pretty good, 225 too tall
...
; decision= decision+ 4 * (x - y) + 10
        ...
        ADDD    decision
        STD     decision
        BRA     CircleLoop_EA          ; Loop until x <= y
; else:
...
; Plot (x + x_Center, y + y_Center)
        LDA     y0+1
        LDB     #CircleScale_EA    ; seems to scale better
        MUL                         ; Multiply A * B
        ADDA    y_Center+1
        LDB     x0+1
        ADDB    x_Center+1
        BSR     DoSetYXCheck_EA    ; Plot the pixel
...
DoSetYXCheck_EA:
    	...
    	BPL     >
        CLRA
!       CMPA    #ScreenHeight_EA-1
        BLE     >
        LDA     #ScreenHeight_EA-1
!       TSTB
        BPL     >
        CLRB
!       CMPB    #ScreenWidth_EA-1
        BLE     >
        LDB     #ScreenWidth_EA-1
!       JMP     SET_EA