...
; Test for a horizontal line
        ...
        LBNE    LineNotHorizontal_SG4  ; If they aren't the same then go draw a line normally
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_SG4       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
!       JSR     SET_SG4      ; Go draw the pixel on screen
...
!       JSR     SET_SG4           ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_SG4       ; If <=, then keep looping
