...
; Test for a horizontal line
        ...
        LBNE    LineNotHorizontal_HIG157  ; If they aren't the same then go draw a line normally
; Get the number of bytes between pixels
        ...
        LBEQ    LineDrawDot1_HIG157    ; If zero then go SET one single pixel
        LDU     startX          ; Otherwise flip the startx and endx coordinates
        STU     endX            ;
        STX     startX          ;

ReadyToDoHorizontalLine_HIG157:
* See if we have more then 16 pixels to draw
!       LDD     endX
        SUBD    startX          ;
        CMPD    #8             
        LBLS    LineNotHorizontal_HIG157  ; If the size is <= 8 then go draw the line normally
...
; Turn pixels into bytes
        ...
        LDB     #BytesPerRow_HIG157 ; MUL A by the number of bytes per row
        MUL      
...
!       PSHS    A               ; save the pixel pattern on the stack
        ...
        LDU     #LineDrawSetTableStart_HIG157
...
* Draw the pixels on the right side of the line
        LDU     #LineDrawSetTableEnd_HIG157
...
; IF error2 < deltaX THEN
        ...
        BLT     >               ; If Less than, then skip ahead
;        BRA     @IFDone         ; Jump to END IF line
        BRA     LINELoop_HIG157       ; Might as well jump to the top of the loop, that's where we'll end up anyway and it saves a few cycles
...
BoxDrawDot0_HIG157:
!       JSR     SET_HIG157      ; Go draw the pixel on screen
...
!       JSR     SET_HIG157      ; Go draw the pixel on screen
        ...
        CMPA    BoxEndY         ; Compare the y coordinate with the end y coordinate
        BLS     BFillLoop_HIG157 ; If <=, then keep looping
