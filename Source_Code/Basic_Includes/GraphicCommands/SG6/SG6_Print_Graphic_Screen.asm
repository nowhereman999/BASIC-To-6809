...
!       LDA     _Var_PF04
        ...
        BSR     AtoGraphics_Screen_SG6
        LDA     #$20        ; Blank
        BRA     AtoGraphics_Screen_SG6 ; Print A on the screen and return, Done
@Print1000sGScreen:
        LDA     _Var_PF01
        ADDA    #$30
        BSR     AtoGraphics_Screen_SG6
...
@Print1sGScreen:
        ...
        BSR     AtoGraphics_Screen_SG6
        LDA     #$20        ; Blank
        BRA     AtoGraphics_Screen_SG6 ; Print A on the screen and return

AtoGraphics_Screen_SG6:
...
!       LDU   BEGGRP    ; U = screen start
        LEAU  Screen_Size_SG6-2*BytesPerRow_SG6,U      ; U = screen End - 2 rows
        PSHS  U         ; Save the value on the stack
        CMPX  ,S++      ; Compare X with the value on the stack, fix the stack
        BLO   >
        LDX   BEGGRP    ; X = Screen Start
        LEAX  Screen_Size_SG6-BytesPerRow_SG6*2,X  ; X = Screen Start + screen size - 1 row = bottom of the screen
        STX   GraphicCURPOS
!
;       CMPA  #$0D      ; Enter Key?
...
;!       CMPD  #BytesPerRow_SG6          ; Is D less than BytesPerRow_SG6?
;        BLO   >             ; If yes, we're done
;        SUBD  #BytesPerRow_SG6          ; Subtract one full row
;        BRA   <             ; Repeat until D < BytesPerRow_SG6
;; B now contains the column number
;!       SUBB   #BytesPerRow_SG6      ; B = B - ByttesPerRow_SG6
;        LDA   #'!    ;#$20      ; Blank
;!       BSR   @GraphicTextDrawAatX  ; Go Draw character A on screen at X
;        LEAX  BytesPerChar,X       ; Move to the next position
;        INCB
;        BNE   <         ; if not keep printing a blank
;        LEAX  2*BytesPerRow_SG6,X    ; Point at the start of the next row
;        LEAU  2*BytesPerRow_SG6,U    ; U = screen End
;        PSHS  U         ; Save the value on the stack
;        CMPX  ,S++      ; Compare X with the value on the stack, fix the stack
;        BLO   @GraphicsTextUpdateCursor
;        LDX   BEGGRP                  ; X = Screen Start
;        LEAX  Screen_Size_SG6-BytesPerRow_SG6*FontHeight,X  ; X = Screen Start + screen size - 1 row = bottom of the screen
;        BRA   @GraphicsTextUpdateCursor  ; Update Cursor and exit
...
!       CMPB    #96             ; Is it a printable character?
        BLO     >               ; Yes, skip ahead
        CLRB                    ; Otherwise, make it a space
!       LDX     #CharJumpTable_SG6 ; Get the address of the character jump table
