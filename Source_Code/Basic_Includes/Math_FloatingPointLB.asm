; Floating point library - by Lennart Benschop
...
; Bug fixed in SCANNUM routine by Glen Hewlett
; A few tweaks to Marco's Adapted version by Glen Hewlett
; Support to handle converting 64 bit unsigned values to floating point (U64_TO_FP) by Glen Hewlett
;
; Permission was granted by Lennart Benschop to include his FP library with my BASIC to 6809 compiler.
; Thanks Lennart
;
; Math functions:
; Routine Name    Function
; ------------    --------
; FPADD         - FLOATING POINT ADDITION  (Float @ -10,U + Float @ -5,U, result Float @ -5,U)
; FPSUB         - FLOATING POINT SUBTRACTION (Float @ -10,U - Float @ -5,U, result Float @ -5,U)
; FPMUL         - MULTIPLY FLOATING POINT NUMBERS (Float @ -10,U * Float @ -5,U, result Float @ -5,U)
; FPDIV         - DIVIDE FLOATING POINT NUMBERS (Float @ -10,U / Float @ -5,U, result Float @ -5,U)
; FPSQRT        - THIS ROUTINE TAKES THE SQUARE ROOT OF AN FP NUMBER SQR(Float @ -5,U) result Float @ -5,U
; FPSIN         - SIN() SIN(Float @ -5,U) result Float @ -5,U
; FPCOS         - COS() COS(Float @ -5,U) result Float @ -5,U
; FPTAN         - TAN() TAN(Float @ -5,U) result Float @ -5,U
; FPATAN        - ATAN() ATAN(Float @ -5,U) result Float @ -5,U
; FPEXP         - EXP() EXP(Float @ -5,U) result Float @ -5,U
; FPLN          - LN() NATURAL LOGARITHM LN(Float @ -5,U) result Float @ -5,U
;
; Other Floating point routines:
; INT2FP        - CONVERT SIGNED 16-BIT INTEGER IN D TO FLOATING POINT @ U
; FP2INT        - CONVERT FP NUMBER TO SIGNED 16-BIT NUMBER IN D
; UFLOAT        - CONVERT UNSIGNED DOUBLE NUMBER AT X TO FLOAT
; UNINT2FP      - CONVERT UNSIGNED 16-BIT INTEGER IN D TO FLOATING POINT
; UINT          - CONVERT FLOAT TO UNSIGNED 32-BIT INTEGER AT X (A IS NONZERO IF NUMBER WAS NOT INTEGER OR ZERO)
; FP2UINT       - CONVERT FP NUMBER TO SIGNED OR UNSIGNED 16-BIT NUMBER IN D. (ACCEPTABLE VALUES ARE -65535..65535)
; FPSCIENT      - CONVERT FP NUMBER TO STRING AT ADDRESS Y IN SCIENTIFIC NOTATION
; SCANNUM       - SCAN A NUMBER AT ADDRESS Y AND CONVERT TO INTEGER OR FLOATING POINT
; FPLOD         - LOAD FP NUMBER FROM ADDRESS X AND PUSH ONTO USER STACK
; FPSTO         - POP FP NUMBER FROM USER STACK AND STORE INTO ADDRESS X
; FPCMP         - COMPARE FLOATING POINT NUMBERS, FLAGS AS WITH UNSIGNED COMPARISON
; INT2BCD       - CONVERT INTEGER AT X TO BCD
; FPFLOOR       - COMPUTE THE FLOOR OF AN FP NUMBER (RESULT IS STILL FP)
; FPEXG         - EXCHANGE TOP TWO NUMBERS ON THE STACK
...
FPOVF		BRA     *       ; Endless Loop
INTRANGE	BRA     *       ; Endless Loop
INVAL		BRA     *       ; Endless Loop
...
;
; FP Library seems to need some space after so reserve a little extra (figure this out later)
FPStackspace	RMB	5*32    ; Lots of space for the U FP Stack
...
; Subroutine to test if FP number @ -5,U is zero (exponent and mantissa all 0)
FPTEST0:
	LDD 	-5,U
	BNE 	>
	LDD 	-3,U
...
; FLOATING POINT ADDITION  (Float @ -10,U + Float @ -5,U, result Float @ -5,U)
DoFPLB_Add_ForCompiler:
	LEAU	12,S	; U points at the stack location where the two floating point numbers are to be added
	BSR	FPADD	; Do the Floating point add
	LEAU	-5,U
	PULU	A,X,Y
	LEAU	5,U
	PSHU	A,X,Y	; Fix result location on the S
	LDU	,S	; Get the return address off the stack
	LEAS	7,S	; Adjust the stack so it now points at the result of the result
	JMP	,U	; Return

; FLOATING POINT Subtraction  (-Float @ -10,U + Float @ -5,U, result Float @ -5,U)
DoFPLB_Subtract_ForCompiler:
	LEAU	7,S	; U points past the stack location where value 2
        JSR   	FPNEG	; Make Value 2 a negative number at -5,U
	LEAU	12,S	; U points past the two FP numbers
	BSR	FPADD	; Do the Floating point add
	LEAU	-5,U
	PULU	A,X,Y
	LEAU	5,U
...
FPSUB:
        JSR FPNEG	; FLOATING POINT NEGATION of number at -5,U
...
FPADD1:
                BSR FPTEST0     ; Subroutine to test if FP number @ -5,U is zero (exponent and mantissa all 0)
...
;		CLR -6,U
                ...
                LDD     #$0000
                STD -10,U	;UNDERFLOW, SUBSTITUTE ZERO.
		STD -8,U
		STA -6,U
...
DoFPLB_Mul_ForCompiler:
	LEAU	12,S	; U points at the numbers on the stack
	BSR	FPMUL	; Do the Multiply
	LDD	,S	; Get return address
	LEAS	7,S	; S now points at the result
	TFR	D,PC	; Return

; MULTIPLY FLOATING POINT NUMBERS. (Float @ -10,U * Float @ -5,U, result Float @ -5,U)
FPMUL:		LDA -9,U
		EORA -4,U
		ANDA #$80
		STA ,-S		;SIGN DIFFERENCE TO STACK.
		JSR FPTEST0     ; Subroutine to test if FP number @ -5,U is zero (exponent and mantissa all 0)
		BEQ FPUNDF	
		LDD -7,U
		BNE FPMULA		
		LDD -9,U
...
DoFPLB_Div_ForCompiler:
	LEAU	12,S	; U points at the numbers on the stack
	BSR	FPDIV	; Do the Multiply
	LDD	,S	; Get return address
	LEAS	7,S	; S now points at the result
	TFR	D,PC	; Return
...
FPNEG:		JSR FPTEST0     ; Subroutine to test if FP number @ -5,U is zero (exponent and mantissa all 0)
...
UFLOAT		LEAU 5,U	;MAKE ROOM FOR EXTRA NUMBER ON STACK.
		LDD ,X          ; Get the MSWord
		STD -4,U        ; Save MSWORD
		LDD 2,X         ; D = the LSWORD
		CLR -5,U        ; Clear Exponent
UF16		STD -2,U	;TRANSFER INTEGER TO FP NUMBER.	
		JSR FPTEST0     ; Subroutine to test if FP number @ -5,U is zero (exponent and mantissa all 0)
		BEQ UFZERO      ; If FP number is zero then, make exponent zero (which it will already be) and jump to Check FP stack doesn't overflow
		LDB #$9F	;NUMBER IS NOT ZERO (%1001 1111) $80 normalized so this is 31 bits to start with
		STB -5,U        ; Save as Exponent value
		TST -4,U        ; TST MSB of the long int
		BMI UFDONE      ; If it's >$7F then Unisgned Float conversion is done
UFLOOP		DEC -5,U	;DECREMENT EXPONENT counter
		ASL -1,U 
		ROL -2,U
		ROL -3,U
		ROL -4,U	;SHIFT MANTISSA
		BPL UFLOOP	;UNTIL NORMALIZED (we have bits in the MS Bit of the MS Byte)
UFDONE		LDB -4,U        ; B = MSB of Manitssa
		ANDB #$7F       ; Clear the MS bit
...
UNINT2FP	CLR ,-S
		BRA I2FP2
; CONVERT SIGNED 16-BIT INTEGER IN D TO FLOATING POINT @ U
...
I2FP2		LEAU 5,U
		...
		CLR -4,U
		CLR -3,U	;CLEAR MSB	
		JSR UF16	;TRANSFER INTEGER TO FP NUMBER.	
		TST ,S+
		BMI FPNEG
...
; A = zero when we get here
		STA ,X		; NUMBER IS ZERO
		STA 1,X
		STA 2,X
		STA 3,X
                INCA		; INDICATE NON-INTEGER.
...
SCANNUM:
                ; CLR DPL         ; CLear the DECIMAL POINT LOCATION (This clear is not needed as SCANINT below clears DPL)
                JSR SKIPSPACE   ; Skip spaces at the beginning of the number
		CLR ,-S		;STORE SIGN ON STACK.
		CMPB #'-'	;TEST FOR MINUS SIGN.
		BNE SN1         ; Skip if not negative
		INC ,S		;SET SIGN ON STACK
		LDB ,Y+         ; Get first digit (after the minus sign)
SN1		JSR SCANINT	;FIRST SCAN THE NUMBER AS AN INTEGER into INTBUF
		LDX #INTBUF     ; X points at 32bit integer
		JSR UFLOAT	;CONVERT TO FLOAT.
		LDB -1,Y        ; Get next byte
SN1LOOP		CMPB #'.'       ; Is it a decimal?
		BNE SN1C        ; If not skip forward this is a really big number
		TST DPL		;IF DECIMAL POINT LOCATION ALREADY SET, ACCEPT NO OTHER POINT.
		BNE SN1D
		INC DPL         ; Increment the DECIMAL POINT LOCATION
		LDB ,Y+
		BRA SN1LOOP
SN1C		SUBB #'0'       ; Is it the end of the string?
		BLO SN1D        ; Skip forward if < 0
		CMPB #9
		BHI SN1D        ; Skip forward if > 9
		CLRA
		JSR INT2FP	;CONVERT DIGIT TO FP @ U
		JSR FPEXG       ; Swap FP numbers
		LDX #FPTEN      ; Get ready to multiply by 10
		JSR FPLOD       ; Setup
		JSR FPMUL	;MULTIPLY ORIGINAL NUMBER BY 10.
		JSR FPADD	;ADD DIGIT TO IT.
		TST DPL         ; Test the DECIMAL POINT LOCATION
		BEQ SN1K
		INC DPL		;ADJUST DECIMAL POINT LOCATION (ONE MORE DIGIT AFTER .)
SN1K		LDB ,Y+
		BRA SN1LOOP		
SN1D		TST ,S+         ; Get the sign, restore the stack
		BEQ SN1A        ; if it's zero then it's positive
		JSR FPNEG	;NEGATE THE NUMBER IF NEGATIVE.
;SN1A		CLR ,-S         ; Clear two bytes on the stack
;		CLR ,-S		;PREPARE EXPONENT PART ON STACK.
SN1A            CLRB
                STB     ,-S
                STB     ,-S

		LDB -1,Y        ; Check if the string is an Exponent
		CMPB #'e'       ; Are we looking at an e?
		BEQ SN1E        ; If so skip forward
		CMPB #'E'       ; Check for capitol E too
...
SN1G		LDB ,Y+
SN1H		LDA DPL         ; Get DECIMAL POINT LOCATION
		PSHS A
		CLR DPL         ; Clear the DECIMAL POINT LOCATION
		INC DPL         ; DECIMAL POINT LOCATION = 1
		JSR SCANINT	;SCAN THE EXPONENT PART.
		PULS A          ; Get original value back
		STA DPL		;RESTORE DECIMAL POINT LOCATION
		LDA INTBUF
		ORA INTBUF+1
		ORA INTBUF+2
...
SN1I		SEX
		STD ,S
SN1F		LDB DPL         ; get value of DECIMAL POINT LOCATION
		BEQ SN1J        ; If it's zero then don't decrement it, skip ahead
		DECB            ; Decrement the DECIMAL POINT LOCATION
SN1J		NEGB            ; Negate the DECIMAL POINT LOCATION
		SEX             ; D = DECIMAL POINT LOCATION
		ADDD ,S++	; ADD EXPONENT PART AS WELL, fix the stack
		PSHS D          ; Save on the stack
		LDX #FPTEN      ; Point at float value of 10
		JSR FPLOD       ; copy to U stack
		PULS D          ; Get D back
		JSR FPIPOWER    ; TAKE 10^-EXP (RAISE FP NUMBER TO AN INTEGER POWER CONTAINED IN D)
		JSR FPMUL       ; Do multiplication
SN1B		RTS		

; SCAN INTEGER NUMBER BELOW 1E9 AT ADDRESS Y, FIRST DIGIT IN B.
;SCANINT:	CLR DPL         ; Clear DECIMAL POINT LOCATION
;SCANINT1	CLR INTBUF
;		CLR INTBUF+1
;		CLR INTBUF+2
;		CLR INTBUF+3	;INITIALIZE NUMBER
SCANINT         CLRA
                STA DPL         ; Clear DECIMAL POINT LOCATION
        	STA INTBUF
		STA INTBUF+1
		STA INTBUF+2
		STA INTBUF+3	;INITIALIZE NUMBER
SNLOOP		CMPB #'.'	
		LBNE SN2A	;TEST FOR DECIMAL POINT.
		TST DPL         ; Check the DECIMAL POINT LOCATION
...
SN2A		SUBB #'0'
		...
		TST DPL         ; TEST DECIMAL POINT LOCATION
		BEQ SN2B        ; If it's zero skip ahead
		INC DPL		;INCREMENT DEECIMAL POINT LOC IF SET.
SN2B		PSHS B          ; Save Digit
		LDD INTBUF+2    ; D = LS Word
		ASLB
		ROLA            ; D = D * 2
		STD INTBUF+2    ; Save as new LS Word
		STD INTBUF2+2   ; Also save in Buff2 as LS Word
		LDD INTBUF      ; Get MS Word
		ROLB            ;
		ROLA            ; D = D * 2
		STD INTBUF      ; Save as new MS Word x 2
		STD INTBUF2     ; Also save in Buff2 as MS Word
		ASL INTBUF+3
		ROL INTBUF+2
		ROL INTBUF+1
		ROL INTBUF      ; Shift Double Int again (2nd time) x 4
		ASL INTBUF+3
		ROL INTBUF+2
		ROL INTBUF+1
		ROL INTBUF      ; Shift Double Int again (3rd time) x 8
		LDD INTBUF+2    ; D = D*8 (LS Word)
		ADDD INTBUF2+2  ; D = D+D*2 (LS Word)
		STD INTBUF+2    ; Save D*10 value (LS Word)
		LDD INTBUF      ; D = D*8 (MS word)
		ADCB INTBUF2+1  
		ADCA INTBUF2    ; D = D+D*2+ Carrybit (MS Word)
		STD INTBUF	; MULTIPLY THE INTEGER BY 10
		LDD INTBUF+2
		ADDB ,S+	;ADD THE DIGIT IN, fix the carry
		ADCA #0         ; Add the carry
		STD INTBUF+2    ; Save updated value
		BCC SN2         ; If the carry is clear, skip ahead
		LDD INTBUF      ; otherwise add 1 to the MS Word
		ADDD #1
		STD INTBUF
SN2		LDD INTBUF      ; D = 
		CMPD #$5F5      
...
FPPOW1		STD ,--S	;STORE THE EXPONENT.
		LDX #FPONE      ; Get ready to mulitply by 1
		JSR FPLOD	;START WITH NUMBER ONE.

FPPOWLOOP	LSR ,S
		ROR 1,S		;DIVIDE EXPONENT BY 2.
		BCC FPPOW2	;TEST IF IT WAS ODD. (If it was even skip forward)
		LEAX -10,U      
		JSR FPLOD
...
FPLN	        JSR FPTEST0     ; Subroutine to test if FP number @ -5,U is zero (exponent and mantissa all 0)
		...
		SUBB #$80	;UNBIAS IT.
		SEX
		JSR INT2FP	;CONVERT TO FP in D @ U
		LDX #FLN2
		JSR FPLOD
...
; Routine to convert signed 64-bit integer to FP
; Enter with:
; X pointing at the signed 64 bit integer
; U pointing at the detination location for the 5 byte FP number
; Exit:
; X - Unchanged
; U = U + 5
; Clobbers D
;
S64_TO_FP:
    LEAU 5,U	         ; MAKE ROOM FOR EXTRA NUMBER ON STACK.
    LDA ,X               ; Load sign byte
    STA SIGN_TEMP        ; Save sign
    LDD ,X               ; Load high word
    STD -4,U             ; Store in mantissa bytes 0-1
    LDD 2,X              ; Load next word
    STD -2,U             ; Store in mantissa bytes 2-3
    LDD 4,X              ; Load next word
    STD ,U               ; Store in mantissa bytes 4-5
    LDD 6,X              ; Load low word
    STD 2,U              ; Store in mantissa bytes 6-7
    LDA SIGN_TEMP        ; Restore sign
    BPL S64_POS          ; If positive, skip negation
    COM -4,U             ; Negate mantissa (two's complement)
    COM -3,U
    COM -2,U
    COM -1,U
    COM ,U
    ...
    STA -4,U
S64_POS:
    JSR TEST_ZERO_64     ; Check if zero
    BEQ S64_FP_ZERO      ; If zero, set FP to 0
    LDB #$BF             ; Set exponent to 191 (128 + 63)
    STB -5,U             ; Store exponent
    TST -4,U             ; Check MSB
    BMI S64_FP_DONE      ; If set, normalization done
S64_FP_LOOP:
    DEC -5,U             ; Decrement exponent
    ASL 3,U              ; Shift mantissa left (LSB to MSB)
    ROL 2,U
    ROL 1,U
    ROL ,U
    ROL -1,U
    ...
    ROL -3,U
    ROL -4,U
    BPL S64_FP_LOOP      ; Until MSB set
S64_FP_DONE:
    LDB -4,U
    ANDB #$7F            ; Remove hidden bit
    STB -4,U
    LDA SIGN_TEMP        ; Apply sign
    BPL S64_FP_END
    LDA -4,U
    ORA #$80             ; Set sign bit
    STA -4,U
S64_FP_END:
    RTS
S64_FP_ZERO:
    CLR -5,U             ; Set exponent to 0
    RTS

; Routine to convert FP to signed 32-bit integer
FP_TO_S32:
    JSR FPTEST0     ; Subroutine to test if FP number @ -5,U is zero (exponent and mantissa all 0)
    BEQ FP_S32_ZERO      ; If zero, set result to 0
    LDA -4,U             ; Load MSB of mantissa
    STA SIGN_TEMP        ; Save sign
    ANDA #$7F            ; Clear sign bit
    STA -4,U             ; Store back
    LDA -5,U             ; Load exponent
    SUBA #128            ; Unbias exponent
    CMPA #31             ; Compare with max shift
    BGT FP_S32_OVERFLOW  ; If exponent > 31, overflow
    STA ,-S              ; Save exponent on stack
    LDA -4,U             ; Load MSB of mantissa
    ORA #$80             ; Set hidden bit
    STA -4,U             ; Store back
    LDX #S32_OUT         ; Point to output
    LDD -4,U             ; Load mantissa bytes 1-2
    STD ,X++             ; Store in result
    LDD -2,U             ; Load mantissa bytes 3-4
    STD ,X               ; Store in result
    LDA ,S+              ; Restore exponent
    BEQ FP_S32_SIGN      ; If exponent = 0, no shift
    TFR X,Y              ; Save X
    LDX #S32_OUT         ; Point to full 32-bit result
FP_S32_SHIFT:
    LSL 3,X              ; Shift left (LSB to MSB)
    ROL 2,X
    ROL 1,X
    ROL ,X
    DECA                 ; Decrease shift count
    BNE FP_S32_SHIFT     ; Repeat until done
FP_S32_SIGN:
    LDA SIGN_TEMP        ; Check sign
    BPL FP_S32_DONE      ; If positive, done
    LDX #S32_OUT         ; Point to result
    LDB #4
    CLRA                 ; Clear carry for subtraction
FP_S32_NEGATE:
    LDA #0
    SBCA ,X               ; Subtract byte from 0
    STA ,X+              ; Store negated byte
    DECB
    BNE FP_S32_NEGATE    ; Repeat for all 4 bytes
FP_S32_DONE:
    RTS
FP_S32_ZERO:
    LDX #S32_OUT         ; Point to output
...
FP_S32_ZERO_LOOP:
    CLR ,X+              ; Clear byte
    DECB
    BNE FP_S32_ZERO_LOOP
    RTS
FP_S32_OVERFLOW:
    LDX #S32_OUT         ; Point to output
    LDA SIGN_TEMP        ; Check sign
    BPL FP_S32_MAX       ; If positive, set to max
    LDD #$8000           ; Set to min value (-2^31)
    STD ,X
    LDD #0
    STD 2,X
    RTS
FP_S32_MAX:
    LDD #$7FFF           ; Set to max value (2^31 - 1)
    STD ,X
    LDD #$FFFF
    STD 2,X
    RTS

; Routine to convert FP to unsigned 32-bit integer
FP_TO_U32:
    JSR FPTEST0     ; Subroutine to test if FP number @ -5,U is zero (exponent and mantissa all 0)
    BEQ FP_U32_ZERO      ; If zero, set result to 0
    LDA -4,U             ; Load MSB of mantissa
    BMI FP_U32_NEG       ; If negative, set to 0
    LDA -5,U             ; Load exponent
    SUBA #128            ; Unbias exponent
    CMPA #31             ; Compare with max shift
    BGT FP_U32_OVERFLOW  ; If exponent > 31, overflow
    STA ,-S              ; Save exponent on stack
    LDA -4,U             ; Load MSB of mantissa
    ANDA #$7F            ; Clear sign bit
    ORA #$80             ; Set hidden bit
    STA -4,U             ; Store back
    LDX #U32_OUT         ; Point to output
    LDD -4,U             ; Load mantissa bytes 1-2
    STD ,X++             ; Store in result
    LDD -2,U             ; Load mantissa bytes 3-4
    STD ,X               ; Store in result
    LDA ,S+              ; Restore exponent
    BEQ FP_U32_DONE      ; If exponent = 0, no shift
    TFR X,Y              ; Save X
    LDX #U32_OUT         ; Point to full 32-bit result
FP_U32_SHIFT:
    LSL 3,X              ; Shift left (LSB to MSB)
    ROL 2,X
    ROL 1,X
    ROL ,X
    DECA                 ; Decrease shift count
    BNE FP_U32_SHIFT     ; Repeat until done
FP_U32_DONE:
    RTS
FP_U32_ZERO:
FP_U32_NEG:
...
FP_U32_ZERO_LOOP:
    CLR ,X+              ; Clear byte
    DECB
    BNE FP_U32_ZERO_LOOP
    RTS


; Convert the highest 32 bits of an unsigned 64 bit number into a floating point number
; If the top 32 bits = 0 then the lower 32 bits will be used for the conversion into a floating point number
; This code is based on Lennart's code for doing 32 bit unsigned conversion above (UFLOAT) but made to handle
; 64 bit Unsinged values, therefore it has not been tested as much as the rest of the code above - Glen
U64_TO_FP:
    	LEAU 	5,U        	; Make room for 5-byte FP number
    	LDD 	,X          	; Load bytes 0-1
    	STD 	-4,U
    	LDD 	2,X         	; Load bytes 2-3
    	STD 	-2,U
    	LDB 	#191        	; Exponent for 63 bits (128 + 63)
    	STB 	-5,U
; Check if high 32 bits are zero
    	...
    	LDD 	-2,U
    	BNE 	UF64_NOT_ZERO
; High 32 are zero, load low 32
    	LDD 	4,X         	; Load bytes 4-5
    	STD 	-4,U
    	LDD 	6,X         	; Load bytes 6-7
    	STD 	-2,U
    	LDB 	#159        	; Exponent for 31 bits (191 - 32)
    	STB 	-5,U
UF64_NOT_ZERO:
    	JSR 	FPTEST0     ; Subroutine to test if FP number @ -5,U is zero (exponent and mantissa all 0)
    	BEQ 	UF64_ZERO
...
; Routine to convert FP to 64-bit unsigned integer
FP_TO_UINT64:
    JSR FPTEST0     ; Subroutine to test if FP number @ -5,U is zero (exponent and mantissa all 0)
    BEQ FP_ZERO_UINT     ; If zero, set result to 0
    LDA -4,U             ; Load MSB of mantissa (sign bit)
    BMI FP_NEG_UINT      ; If negative, handle as error (set to 0)
    LDA -5,U             ; Load exponent
    SUBA #128            ; Unbias exponent (e - 128)
    CMPA #63             ; Compare with max shift for 64-bit
    BGT FP_OVERFLOW_UINT ; If exponent > 63, overflow
    STA ,-S              ; Save exponent on stack
    LDA -4,U             ; Load MSB of mantissa
    ANDA #$7F            ; Clear sign bit
    ORA #$80             ; Set hidden bit
    STA -4,U             ; Store back
    LDX #UINT64_OUT      ; Point to output buffer
    LDB #8               ; 8 bytes to clear
FP_CLEAR_UINT:
    CLR ,X+              ; Clear byte
    DECB
    BNE FP_CLEAR_UINT
    LDX #UINT64_OUT+4    ; Point to lower 4 bytes of result
    LDD -4,U             ; Load mantissa bytes 1-2
    STD ,X++             ; Store in result
    LDD -2,U             ; Load mantissa bytes 3-4
    STD ,X               ; Store in result
    LDA ,S+              ; Restore exponent
    BEQ FP_UINT_DONE     ; If exponent = 0, no shift needed
    TFR X,Y              ; Save X
    LDX #UINT64_OUT      ; Point to full 64-bit result
FP_UINT_SHIFT:
    LSL 7,X              ; Shift left (LSB to MSB)
    ROL 6,X
    ROL 5,X
    ROL 4,X
    ROL 3,X
    ...
    ROL ,X
    DECA                 ; Decrease shift count
    BNE FP_UINT_SHIFT    ; Repeat until done
FP_UINT_DONE:
    RTS
FP_ZERO_UINT:
    LDX #UINT64_OUT      ; Point to output
    LDB #8
FP_ZERO_LOOP_UINT:
    CLR ,X+              ; Clear byte
    DECB
    BNE FP_ZERO_LOOP_UINT
    RTS
FP_NEG_UINT:
FP_OVERFLOW_UINT:
    BRA FP_ZERO_UINT     ; On negative or overflow, return 0

; Routine to convert FP to 64-bit signed integer
FP_TO_SINT64:
    JSR FPTEST0     ; Subroutine to test if FP number @ -5,U is zero (exponent and mantissa all 0)
    BEQ FP_ZERO_SINT     ; If zero, set result to 0
    LDA -4,U             ; Load MSB of mantissa
    STA SIGN_TEMP        ; Save sign
    ANDA #$7F            ; Clear sign bit
    STA -4,U             ; Store back
    LDA -5,U             ; Load exponent
    SUBA #128            ; Unbias exponent
    CMPA #63             ; Compare with max shift
    BGT FP_OVERFLOW_SINT ; If exponent > 63, overflow
    STA ,-S              ; Save exponent on stack
    LDA -4,U             ; Load MSB of mantissa
    ORA #$80             ; Set hidden bit
    STA -4,U             ; Store back
    LDX #SINT64_OUT      ; Point to output buffer
    LDB #8
FP_CLEAR_SINT:
    CLR ,X+              ; Clear byte
    DECB
    BNE FP_CLEAR_SINT
    LDX #SINT64_OUT+4    ; Point to lower 4 bytes
    LDD -4,U             ; Load mantissa bytes 1-2
    STD ,X++             ; Store in result
    LDD -2,U             ; Load mantissa bytes 3-4
    STD ,X               ; Store in result
    LDA ,S+              ; Restore exponent
    BEQ FP_SINT_SIGN     ; If exponent = 0, no shift
    TFR X,Y              ; Save X
    LDX #SINT64_OUT      ; Point to full 64-bit result
FP_SINT_SHIFT:
    LSL 7,X              ; Shift left (LSB to MSB)
    ROL 6,X
    ROL 5,X
...
FP_SINT_SIGN:
    LDA SIGN_TEMP        ; Check sign
    BPL FP_SINT_DONE     ; If positive, done
    LDX #SINT64_OUT      ; Point to result
    LDB #8
    CLRA                 ; Clear carry for subtraction
FP_SINT_NEGATE:
    LDA #0
    ...
    STA ,X+              ; Store negated byte
    DECB
    BNE FP_SINT_NEGATE   ; Repeat for all 8 bytes
FP_SINT_DONE:
    RTS
FP_ZERO_SINT:
    LDX #SINT64_OUT      ; Point to output
    LDB #8
FP_ZERO_LOOP_SINT:
    CLR ,X+              ; Clear byte
    DECB
    BNE FP_ZERO_LOOP_SINT
    RTS
FP_OVERFLOW_SINT:
    LDX #SINT64_OUT      ; Point to output
    LDA SIGN_TEMP        ; Check sign
    BPL FP_SINT_MAX      ; If positive, set to max
    LDD #$8000           ; Set to min value (-2^63)
    STD ,X
    LDD #0
    STD 2,X
    STD 4,X
    ...
    RTS
FP_SINT_MAX:
    LDD #$7FFF           ; Set to max value (2^63 - 1)
    STD ,X
    LDD #$FFFF
    STD 2,X
    STD 4,X
