...
@Loop1  LDA     #127            ; Underscore cursor
        JSR     PrintA_On_Screen   ; Go print A on screen @ Cursor Position
        DEC     W_CURPOS        ; decrement the x co-ordinate, set next position to print on screen on top of the cursor
        DEC     W_CURPOS        ; decrement the x co-ordinate, set next position to print on screen on top of the cursor
!       JSR     KEYIN
...
!       LDA     ,X++            ; Get the next character to print
        ...
        INCB                    ; Increment the buffer size counter
        JMP     @Loop1          ; Go get some more input
