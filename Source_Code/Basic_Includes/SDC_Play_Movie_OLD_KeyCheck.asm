...
NTMovSaveFPS            EQU   _Var_PF07+1                   ; Save the frame rate per second
;
...
NTMovJumpSectorRateNum  EQU   NTMovAudSectors+1 ; word - to calc where to jump to for playback
      ...
      LDX   #SDCNTSAudioBuffer0
      LDA   NTMovFPS,X                    ; Get the frame rate of the movie
      STA   >NTMovSaveFPS                 ; Save it for use later (calculating jump points in the movie)
...
; D = TargetSeconds to resume playback of movie from
; Set position to play and continue
; StartSector = 1 + ((TargetSeconds * FPS) \ 32) * BuildJumpSectorRateNum
NTMov_PlayFromDSeconds:
      PSHS  D                 ; Save Target Seconds on the stack
      CLRA
      LDB   >NTMovSaveFPS     ; Get the FPS
