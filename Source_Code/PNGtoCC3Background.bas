...
99 Print: Print "PNG to CoCo 3, 16 Colour Background Converter v"; VersionNumber$; " by Glen Hewlett"
Print "Converts a PNG image file into a CoCo 3 compatible Background that can be LOADMed on a CoCo 3"
Print "There are size restrictions on the PNG image:"
Print "The PNG width must be 256 or more pixels and a multiple of 256 pixels.  Example a width of 256 x 7 = 1792 is fine but a width of 1700"
Print "will be trucated to 256 x 6 = 1536 pixels wide."
...
Print "truncated to 64 x 6 = 384 pixels tall."
Print
Print "Usage: PNGtoCC3Background [-dx] [-makepal] [-blkxxx] [-big] InName.png"
Print "Where:"
Print "InName.png is a 32 bit RGBA (includes transparency) png file"
Print "-dx      - Selects Dither method to use for the background image"
Print "           Where x is:"
Print "           0 = No Dithering"
Print "           1 = Floyd-Steinberg Dithering (Default)"
Print "           2 = Blue Noise Mask Dithering"
Print "           3 = Ordered Dithering"
Print "-makepal - During the background conversion it will also create a palette file called CoCo3_Palette.asm for use with other"
Print "           images and other CoCo 3 programs."
Print "           If this option isn't given, it will use the CoCo3_Palette.asm file and use those colours for use with creating the background"
Print "-blkxxx    The starting 8k block in the CoCo 3 RAM to start saving the background image (default is 128)"
Print "-big     - Make one BIG .BIN file to be loaded directly off the CoCoSDC using the BASIC command SDC_BIGLOADM"
System
100 Verbose = 0
MakePal = 0
BlkStart = 128
...
For check = 1 To count
    N$ = Command$(check)
    If LCase$(Left$(N$, 4)) = "-big" Then BigLoadm = 1: GoTo 120
...
Print "Generating a Background image"
Print "Width:"; imageWidth
Print "Height:"; imageHeight
If MakePal = 1 Then
    Print "Request to make a CoCo3_Palette.asm palette, calculating the best 16 colours to use..."
Else
    Print "Using palette CoCo3_Palette.asm to calculate the best 16 colours to use..."
End If
...
Print #1, "; Palette file generated by PNGtoCC3Background Version "; VersionNumber$
