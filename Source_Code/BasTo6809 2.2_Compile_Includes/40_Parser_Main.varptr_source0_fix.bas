...
'' -----------------------------------------------------------------------------
'' NOTE:
'' Make sure your global CONSTs (NT_* and TK_ADDR_ONSTACK) are defined in your
'' normal constants section near the top of the whole program, BEFORE this
'' parser code. Otherwise QB64 can treat them as implicit variables (value 0),
'' which leads to ConvertLastType2NVT being called with NVT=0.
...
If Asc(Left$(Temp$, 1)) = &HFA Then
    ...
    ' Robustness: some paths can accidentally leave the type byte as 0.
    ' Treat that as a 16-bit address/value so ConvertLastType2NVT doesn't
    ' explode with "Invalid source type 0".
    If PushedType = 0 Then PushedType = NT_UInt16
    Return
End If
